...
describe("playground webview shell", () => {
  it("renders the minimal composer-first shell with composer-integrated Binary controls", () => {
    ...
    expect(html).toContain("Ask Streaming Binary for anything, @ for files, / for commands");
    ...
    expect(html).toContain(".live-message-transcript{margin-top:10px;max-height:min(38vh,320px);overflow:auto");
    ...
    expect(html).toContain('.workspace-shell[data-binary-hero="true"] .message.live-binary.hero-collapsed .live-message-transcript{max-height:min(62vh,520px)}');
    ...
    expect(html).toContain(".binary-panel-body{order:3;display:flex;flex-direction:column;gap:0;min-height:0;max-height:min(50vh,440px);overflow-y:auto");
    ...
    expect(html).toContain("radial-gradient(circle at 18% 20%");
  });
});
