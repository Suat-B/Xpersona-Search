...
function buildQwenPrompt(input) {
    ...
    if (input.workspaceRoot)
        summaryLines.push(`- Workspace root: ${input.workspaceRoot}`);
    if (input.preview.resolvedFiles.length) {
        summaryLines.push(`- Likely target files: ${input.preview.resolvedFiles.join(", ")}`);
    }
    if (input.preview.activeFile)
        summaryLines.push(`- Active file: ${input.preview.activeFile}`);
    if (input.preview.openFiles.length)
        summaryLines.push(`- Open files: ${input.preview.openFiles.join(", ")}`);
    ...
    sections.push(input.workspaceRoot
    ...
    sections.push("When the user refers to a file by name, start with the matched files listed above. If you still need context, inspect the workspace with read-only tools or safe inspection commands before answering.");
    sections.push(input.mode === "plan"
