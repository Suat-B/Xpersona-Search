...
<html>
  ...
  <body>
    ...
    <script nonce="${n}">
      ...
      function mountRecoveryComposer(reasonText) {
        if (document.getElementById("__xp_recovery_shell")) return;
        ...
        title.textContent = "Playground recovery mode";
        title.style.cssText = "font-size:13px;font-weight:700;letter-spacing:.02em;";

        const note = document.createElement("div");
        const reason = String(reasonText || "webview bootstrap issue").trim();
        note.textContent = "The UI hit an error and switched to safe send mode. " + reason.slice(0, 220);
        note.style.cssText = "font-size:12px;color:#b1b1b1;line-height:1.45;";

        const ta = document.createElement("textarea");
        ta.id = "__xp_recovery_input";
        ta.placeholder = "Type message (Enter to send, Shift+Enter for newline)";
        ta.style.cssText = [
          "width:100%",
          "min-height:94px",
          "max-height:220px",
        ...
        row.style.cssText = "display:flex;justify-content:flex-end;gap:8px;align-items:center;";
        ...
        const postRecoverySend = () => {
          const text = String(ta.value || "").replace(/\r?\n+$/g, "").trim();
          if (!text) return;
          v.postMessage({
            type: "send",
            text,
        ...
        send.addEventListener("click", postRecoverySend);
      ...
      window.addEventListener(
        "error",
        (ev) => {
          const reason = ev && (ev.message || (ev.error && ev.error.message)) ? (ev.message || ev.error.message) : "runtime error";
          mountRecoveryComposer(reason);
        },
        true
      );
