...
function hasQuestionIntent(text: string): boolean {
  return (
    /\b(why|what|where|how|explain|debug|investigate|inspect|trace|check|used for|is this)\b/.test(text) ||
    text.includes("?")
  );
}

function hasDirectRuntimeMarker(text: string): boolean {
  return (
    text.includes("@qwen-code") ||
...
function containsStrongRuntimeNarrativeNoise(text: string): boolean {
  ...
  if (
    normalized.includes("shared a file path related to") &&
    /\b(qwen|sdk|extension)\b/.test(normalized)
  ) {
    return true;
  }
  if (
    (normalized.includes("shared a path to") ||
  ...
  if (
    normalized.includes("i can see you've provided") &&
    /\b(file path|qwen|sdk|cli)\b/.test(normalized)
  ) {
    return true;
  }
  if (
    normalized.includes("i notice you've shared") &&
    /\b(path|qwen|sdk|installation|extension)\b/.test(normalized)
  ) {
    return true;
  }
  if (/\b(qwen|sdk)\b/.test(normalized) && /\binstallation\b/.test(normalized)) return true;
...
function containsSoftRuntimeNarrativeNoise(text: string): boolean {
  const normalized = normalizeRuntimeText(text);
  if (!normalized) return false;

  return (
    normalized.includes("qwen code runtime") ||
...
export function isExplicitRuntimeTask(task: string): boolean {
  const normalized = normalizeRuntimeText(task);
  if (!normalized) return false;
  if (hasDirectRuntimeMarker(normalized) && hasQuestionIntent(normalized)) return true;
  if (
    (normalized.includes("qwen code") || normalized.includes("qwen sdk") || normalized.includes("@qwen-code")) &&
...
export function isWorkspaceQwenTopic(input: {
  task: string;
  ...
  if (targetHasQwen) return true;
  if (!/\bqwen\b/.test(normalizedTask)) return false;
  return !hasDirectRuntimeMarker(normalizedTask);
}

export function containsRuntimeNoiseForContext(input: {
  text: string;
  task: string;
