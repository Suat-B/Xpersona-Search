...
import {
  ...
  NO_CONTENT_DELTA_REASON_PREFIX,
...
function modelLabelForUi(model: string | null | undefined, catalog?: ModelCatalogEntry[]): string {
  ...
  if (selected === DEFAULT_PLAYGROUND_MODEL) return PUBLIC_PLAYGROUND_MODEL_NAME;
...
type ExecutionPolicy = "full_auto" | "yolo_only" | "preview_first";
type ToolRecoveryStage = "none" | "repair" | "tool_enforcement" | "single_file_rewrite" | "fallback";
type ToolRoute = "native_tools" | "text_actions" | "deterministic_synthesis";
type ToolActionSource = "none" | "native_tool_calls" | "structured_json" | "single_file_rewrite_fallback" | "deterministic_synthesis";
...
type RunToolState = {
  ...
  lastFailureCategory?: ToolFailureCategory | null;
};
type PendingAction =
...
function mapLocalApplyCategoryToToolFailureCategory(
  category: LocalApplyFailureCategory | null | undefined
): ToolFailureCategory {
  return category === "no_content_delta" ? "no_content_delta" : "local_apply_failed";
}

function summarizeToolState(toolState: RunToolState | null | undefined): string {
...
function describeToolFailureScope(category: ToolFailureCategory | null | undefined): string {
  if (category === "validation_failed") return "validation";
  if (category === "no_content_delta" || category === "local_apply_failed") return "local_apply";
  if (category) return "backend_generation";
  return "unknown";
}
...
function collectWrappedPayloadCandidates(text: string): string[] {
  ...
  if (fenced?.[1]) pushCandidate(fenced[1]);
...
function looksLikeWrappedToolPayloadText(text: string): boolean {
  ...
  for (const candidate of candidates) {
    if (looksLikeWrappedToolEnvelope(candidate)) return true;
  }
  return false;
}
...
function patchHasLeakedPatchArtifacts(patchText: string): boolean {
  const text = String(patchText || "").trim();
  if (!text) return false;
  if (!patchContainsLeakedPatchArtifacts(text)) return false;
  const recovered = recoverUnifiedDiffFromLeakedPatchArtifacts(text);
  return !(recovered && recovered.trim());
}

function normalizeIncomingPatchText(patchText: string): { patch: string; recovered: boolean } {
  const raw = String(patchText || "").trim();
  if (!raw) return { patch: "", recovered: false };
  const wrapped = recoverUnifiedDiffFromWrappedPayload(raw);
  const primary = wrapped && wrapped.trim() ? wrapped.trim() : raw;
  const leaked = recoverUnifiedDiffFromLeakedPatchArtifacts(primary);
...
class Provider implements vscode.WebviewViewProvider {
  ...
  private pinnedThreadIds = new Set<string>();
  private sessionId: string | null = null;
  private hasShownHomepage = false;
  ...
  private lastAutonomyProgressFingerprint: string | null = null;
  private consecutiveAutonomyProgressRepeats = 0;
  ...
  private lastPlanModeNoticeAt = 0;
  private mentionCatalog: Array<{ path: string; kind: "file" | "folder" }> = [];
  private mentionCatalogAt = 0;
  ...
  private defaultModelAlias = DEFAULT_PLAYGROUND_MODEL;
  ...
  private isFileInfoQuestion(task: string): boolean {
  ...
  constructor(private ctx: vscode.ExtensionContext) {
    ...
    this.hasShownHomepage = ctx.workspaceState.get<boolean>(HOMEPAGE_DISPLAYED_KEY, false);
    ...
    const pinned = ctx.workspaceState.get<string[]>(PINNED_THREADS_KEY, []);
    if (Array.isArray(pinned)) {
      for (const id of pinned) {
        if (typeof id === "string" && id.trim()) this.pinnedThreadIds.add(id.trim());
      }
    }
  ...
  private vscodeCallbackAuthority(): string {
  ...
  private async getStoredApiKey(): Promise<string | null> {
    const readSecret = async (secretKey: string): Promise<string | null> => {
      try {
    ...
    const fallback = String(this.ctx.globalState.get<string>(API_KEY_FALLBACK_STATE_KEY, "") || "").trim();
  ...
  private async postModelCatalog(): Promise<void> {
    const visibleModels =
      this.modelCatalog.find((entry) => entry.alias === this.defaultModelAlias)
  ...
  async handleAuthCallback(uri: vscode.Uri): Promise<void> {
    ...
    if (authority !== this.vscodeCallbackAuthority()) return;
  ...
  private getExecutionPolicy(): ExecutionPolicy {
    const configured = vscode.workspace
      .getConfiguration("xpersona.playground")
      .get<string>(EXECUTION_POLICY_CONFIG_KEY);
  ...
  private mentionsEnabled(): boolean {
    return vscode.workspace.getConfiguration("xpersona.playground").get<boolean>(MENTIONS_ENABLED_FLAG) ?? true;
  }

  private deriveThreadTitle(input: string): string {
  ...
  private sortOpenThreadIds(ids: string[]): string[] {
    return ids.slice().sort((a, b) => {
      const aPinned = this.pinnedThreadIds.has(a) ? 1 : 0;
      const bPinned = this.pinnedThreadIds.has(b) ? 1 : 0;
      if (aPinned !== bPinned) return bPinned - aPinned;
      const aTime = this.threads[a]?.updatedAt ? new Date(this.threads[a].updatedAt as string).getTime() : 0;
      const bTime = this.threads[b]?.updatedAt ? new Date(this.threads[b].updatedAt as string).getTime() : 0;
      return bTime - aTime;
    });
  }

  private async persistPinnedThreads() {
    await this.ctx.workspaceState.update(PINNED_THREADS_KEY, Array.from(this.pinnedThreadIds));
  }

  private async setThreadPinned(id: string, pinned: boolean) {
    const threadId = String(id || "").trim();
    if (!threadId) return;
    if (pinned) this.pinnedThreadIds.add(threadId);
    else this.pinnedThreadIds.delete(threadId);
    await this.persistPinnedThreads();
    await this.postThreadState();
  }

  private async searchWorkspaceMentions(rawQuery: string, rawLimit: number): Promise<Array<{ path: string; kind: "file" | "folder" }>> {
    ...
    const catalogFresh = Date.now() - this.mentionCatalogAt < 15_000;
    if (!catalogFresh || this.mentionCatalog.length === 0) {
      const files = await vscode.workspace.findFiles(
        new vscode.RelativePattern(root, "**/*"),
        new vscode.RelativePattern(root, "**/{node_modules,.git,.next,dist,build}/**"),
        3000
      );
      const folderSet = new Set<string>();
      const items: Array<{ path: string; kind: "file" | "folder" }> = [];
      ...
      for (const folder of folderSet) items.push({ path: folder, kind: "folder" });
    ...
    const rank = (candidate: string) => {
      ...
      if (!query) return 1000 - c.length;
      if (base === query) return 1300 - c.length;
      if (c === query) return 1200 - c.length;
      if (base.startsWith(query)) return 1100 - c.length;
      if (c.startsWith(query)) return 900 - c.length;
      if (c.split("/").some((segment) => segment.startsWith(query))) return 700 - c.length;
      if (base.includes(query)) return 620 - c.length;
      if (c.includes(query)) return 500 - c.length;
      const qParts = query.split("/").filter(Boolean);
      if (qParts.length && qParts.every((part) => c.includes(part))) return 350 - c.length;
      return -1;
    };
  ...
  async cycleMode() {
    const nextMode: Mode = this.mode === "auto" ? "plan" : this.mode === "plan" ? "yolo" : "auto";
    await this.setMode(nextMode);
  }

  async setMode(m: Mode) {
    const normalized: Mode = m === "plan" || m === "yolo" ? m : "auto";
  ...
  private buildNoEditApplyRetryTask(input: {
    objective: string;
    blocker: string;
  ...
  private async runQuickValidationForFile(input: {
    ...
    if (snapshot.existed ? currentContent === snapshot.content : currentContent.length === 0) {
      ...
      return { status: "failed", reason: `${NO_CONTENT_DELTA_REASON_PREFIX}${rel}`, commandsRun: 0, commands: [] };
    }
    ...
    if (plan.coverage === "sanity_only") {
      input.addExecutionEvent?.(
        "quick_validation_sanity_only",
        `${rel}: no specialized validation runner configured; using git diff sanity only.`,
        "info"
      );
    }
  ...
  private publishLocalApplyFailureMeta(input: {
    ...
    const nextToolState: RunToolState = {
      ...((this.lastRunMeta?.toolState || {}) as RunToolState),
  ...
  private mergeToolStatePatch(patch: Partial<RunToolState>, threadId?: string | null) {
    this.lastRunMeta = {
      ...(this.lastRunMeta || {}),
      toolState: {
        ...((this.lastRunMeta?.toolState || {}) as RunToolState),
  ...
  private async attemptLocalApplyRecovery(input: {
    ...
    const nextStage = nextLocalRecoveryStage(input.failure.attemptedStages, input.failure.path, input.failure.category);
    if (!nextStage) {
  ...
  private async askSingleCycle(
    ...
    if (requestedThreadId && requestedThreadId !== this.activeThreadId) {
      ...
      if (existing) this.upsertThread(existing, true);
    ...
    const noActionIntent =
      !this.hasExecutionIntent(text) && !this.hasExplicitEditIntent(text) && !this.hasExplicitCommandRunIntent(text) && !this.hasCodeTaskSignals(text);
    if ((conversational || strictConversationOnly || noActionIntent) && !wantsEdits) {
      ...
      this.cancelRequested = true;
      const cancel: unknown = this.activeStreamCancel;
        ...
        stream(
          ...
          async (ev, p) => {
            if (ev === "token") {
              ...
              const forceTargetPath =
                wantsEdits && hintedTargetPath && !conversational ? hintedTargetPath : null;
                  ...
                  if (type === "edit") {
                    ...
                    const patch =
                      typeof (action as { patch?: unknown }).patch === "string"
                        ...
                        : typeof (action as { diff?: unknown }).diff === "string"
    ...
    const actionabilitySummary = (
      ((this as unknown as { lastRunMeta?: RunMeta | null }).lastRunMeta || null) as RunMeta | null
    )?.actionability?.summary;
    const shouldAutoContextRetry =
      allowActions &&
      wantsEdits &&
      !conversational &&
      !strictConversationOnly &&
    ...
    if (
      ...
      addDiagnosticEvent(
        "no_actions_retry",
        `No actions were produced; retrying with explicit target: ${hintedTargetPath}`,
        "warn"
      );
      this.postRun(runThreadId, {
        type: "status",
    ...
    if (this.pendingActions.length > 0) {
      ...
      const implementationCommandActions = this.pendingActions.filter(
        (a): a is PendingAction & { type: "command"; category?: "implementation" | "validation" } =>
          a.type === "command" && a.category === "implementation"
      );
      const validationCommandActions = this.pendingActions.filter(
        (a): a is PendingAction & { type: "command"; category?: "implementation" | "validation" } =>
          a.type === "command" && a.category !== "implementation"
      );
      ...
      const commandPolicyResolved =
        meta.toolState?.commandPolicyResolved || this.getAutonomyProfile().commandPolicy;
  ...
  async executePendingActions(actions?: PendingAction[], threadIdOverride?: string, traceIdOverride?: string) {
    ...
    const collapsedActions = collapseConflictingFileActions(dedupedActionList);
    ...
    if (collapsedActions.collapsedPaths.length > 0) {
      ...
      addExecutionEvent(
        "collapsed_conflicting_file_actions",
        `Kept only the latest file mutation for: ${detail}.`,
        "warn"
      );
      this.postRun(runThreadId, {
        type: "status",
    ...
    const explicitCommandTask =
      this.hasExplicitCommandRunIntent(taskText) || (!expectedFileChanges && this.hasExecutionIntent(taskText));
    const commandPolicyResolved =
      this.lastRunMeta?.toolState?.commandPolicyResolved || this.getAutonomyProfile().commandPolicy;
    ...
    const r = await req<any>("POST", `${base()}/api/v1/playground/execute`, auth, {
      ...
      actions: actionList.map((a) => {
        if (a.type === "edit") return { type: "edit", path: a.path, patch: a.patch };
        if (a.type === "command") return { type: "command", command: a.command, ...(a.category ? { category: a.category } : {}) };
        if (a.type === "mkdir") return { type: "mkdir", path: a.path };
    ...
    const approvedImplementationCommands: string[] = [];
    const approvedValidationCommands: string[] = [];
      ...
      if (row.action.type === "edit") {
        ...
        if (applied.status === "applied" || applied.status === "partial") {
          if (applied.changed) {
            ...
            if (validation.status === "failed" && isNoContentDeltaReason(validation.reason)) {
    ...
    const skippedApprovedActionsAfterHalt =
      haltedAtResultIndex !== null
        ...
        : 0;
    const shouldRunImplementationCommands =
      approvedImplementationCommands.length > 0 &&
      commandPolicyResolved === "run_until_done" &&
      !haltedValidation &&
      ...
      addExecutionEvent(
        commandPolicyResolved !== "run_until_done"
          ...
          : haltedValidation
            ...
            : "implementation_skipped_no_file_change",
        skippedMessage,
        "warn"
      );
      this.postRun(runThreadId, {
    ...
    let noEditRetryTargetPath: string | null = null;
    ...
    if (expectedFileChanges && changedPaths.size === 0) {
      const existingMissing = Array.isArray(this.lastRunMeta?.missingRequirements)
        ...
        : [];
      const localApplyFailureMeta =
        primaryLocalApplyFailure && primaryLocalApplyFailure.path
          ? {
            ...
            retryStage: "final_failure" as const,
          ...
          : this.lastRunMeta?.localApplyFailure ||
            (noEditRetryTargetPath
              ? {
                ...
                category: "unknown_apply_failure" as const,
                reason:
                  noEditFailureSummary ||
      ...
      if (request && !request.options.noEditApplyRetryAttempted) {
        addExecutionEvent(
          "no_edit_apply_retry",
          "No local file edits were applied; retrying once with local failure telemetry.",
          "warn"
        );
        this.postRun(runThreadId, {
          type: "status",
  ...
  private async runApprovedCommand(
    ...
    const result = await new Promise<{ ok: boolean; exitCode: number | null; stdout: string; stderr: string }>((resolve) => {
      execFile(shell, args, { cwd, timeout: timeoutMs, windowsHide: true, maxBuffer }, (error, stdout, stderr) => {
        const exitCodeRaw = (error as { code?: number | string } | null)?.code;
        const exitCode =
        ...
        const errText = String(stderr || (error as { message?: string } | null)?.message || "");
        resolve({ ok, exitCode: Number.isFinite(exitCode) ? exitCode : null, stdout: out, stderr: errText });
      });
    });
    const summary = `${result.ok ? "APPROVED" : "REJECTED"} command ${command} [exit ${result.exitCode ?? "?"}]${
  ...
  private killAllTerminals(reason?: string) {
  ...
  private postRun(threadId: string | null | undefined, m: any) {
...
function stream(
  ...
  return new Promise((resolve, reject) => {
    ...
    const isBenignTerminalReset = (error: unknown): boolean => {
      const message =
