// Acquire the VS Code webview API safely. If this throws (rare, but possible during webview init),
      ...
      function renderContextSelectionPanel(payload) {
        // Hide the "Why this context / Retrieval" panel entirely.
        if (contextSelectionPanel) contextSelectionPanel.classList.add("hidden");
        return;
      ...
      function sendCurrent() {
        try {
          ...
          if (!currentThreadId) {
            // First-send UX: let the host/backend create a new thread implicitly.
            // This avoids a "new page" jump where the first prompt is queued but never flushed.
