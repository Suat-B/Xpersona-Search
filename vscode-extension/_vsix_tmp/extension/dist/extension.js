...
class Provider {
    ...
    async refreshModelCatalog(auth) {
        ...
        const response = await req("GET", `${base()}/api/v1/playground/models`, requestAuth).catch(() => null);
        const models = Array.isArray(response?.data?.models)
            ? response.data.models
                ...
                certification: item?.certification === "tool_ready" || item?.certification === "chat_only" || item?.certification === "experimental"
                    ? item.certification
                    : undefined,
                ...
                .filter((item) => item.alias && item.displayName && item.model)
            : [];
    ...
    async askSingleCycle(text, parallel, model = DEFAULT_PLAYGROUND_MODEL, reasoning = "medium", options = {}) {
        ...
        if (allowActions &&
	        ...
	        // If the user clearly asked for edits but the model produced no actionable file/command actions,
	        // surface a clear outcome instead of silently ending after diagnostics retries.
	        if (allowActions && wantsEdits && this.pendingActions.length === 0 && guardrailIssues.length === 0) {
	            const retrySuffix = options.contextRetryAttempted ? " (after retry)" : "";
	            const hint = hintedTargetPath ? ` Target: ${hintedTargetPath}.` : "";
	            this.postRun(runThreadId, {
	                type: "status",
	                text: `No actionable edits were produced${retrySuffix}.${hint} Try rephrasing the request with a specific change, or select the exact code to edit.`,
	            });
	            const outcome = {
	                filesChanged: 0,
	                checksRun: 0,
	                quality: "needs_attention",
	                summary: `No actionable edits were produced${retrySuffix}.`,
	                appliedFiles: [],
	            };
	            this.lastActionOutcome = outcome;
	            this.postRun(runThreadId, { type: "actionOutcome", data: outcome });
	            this.pendingActions = [];
	            this.post({ type: "pendingActions", count: 0 });
	            emitDiagnosticsBundle("actions", `No actions were produced for edit intent${retrySuffix}.`);
	            return;
	        }
	        if (this.pendingActions.length > 0) {
  ...
  <head>
    ...
    <style nonce="${n}">
      ...
      * { box-sizing: border-box; }
      ...
      .action-menu.page-mode .sheet-head,
      .action-menu.page-mode .sheet-head-actions,
      .action-menu.page-mode .sheet-grid,
      .action-menu.page-mode .sheet-card,
      .action-menu.page-mode .sheet-row,
      .action-menu.page-mode .tool-toggle,
      .action-menu.page-mode .api-key-row {
        min-width: 0;
      }
      .action-menu.page-mode .sheet-toggle .tool-toggle {
        flex-wrap: wrap;
        align-items: flex-start;
        justify-content: flex-start;
        gap: 6px;
      }
      .action-menu.page-mode .sheet-toggle .tool-toggle > span {
        flex: 1 1 100%;
        min-width: 0;
      }
      .action-menu.page-mode .composer-select {
        width: 100%;
        max-width: 100%;
        min-width: 0;
      }
      ...
      .action-menu.page-mode .api-key-input {
        min-width: 0;
        width: 100%;
      }
      ...
      .chat-panel.active #chatDock .composer-shell textarea::placeholder {
        color: color-mix(in srgb, var(--rep-fg) 34%, transparent) !important;
	      ...
	      .chat-panel.active #chatDock .input-actions.minimal {
	        ...
	        /* Keep the bottom bar from visually "squaring off" the rounded composer corners. */
	        background: transparent !important;
