...
const instruction_bridge_1 = require("./instruction-bridge");
...
function safeJoin(relPath) {
    ...
    if (!joined.startsWith(workspaceRoot))
        return null;
    return joined;
}
function runRg(query, limit = 20) {
    const result = (0, child_process_1.spawnSync)("rg", ["-n", "-i", "-S", "--hidden", "-g", "!node_modules", "-g", "!.git", query, "."], {
        cwd: workspaceRoot,
        encoding: "utf8",
        windowsHide: true,
    });
...
async function handleToolsCall(name, args) {
    ...
    if (name === "workspace.read_file") {
        ...
        if (!target || !fs.existsSync(target))
            return { ...buildTextContent("File not found."), isError: true };
    ...
    if (name === "workspace.validation_plan") {
        const relPath = normalizeRelPath(args.path);
        if (!relPath)
            return { ...buildTextContent("Invalid path."), isError: true };
        const ext = path.extname(relPath).toLowerCase();
        const commands = [`git diff --check -- ${relPath}`];
        if ([".ts", ".tsx", ".js", ".jsx"].includes(ext))
            commands.push(`npm run lint -- ${relPath}`);
        if (ext === ".py")
            commands.push(`python -m py_compile ${safeJoin(relPath)}`);
    ...
    if (name === "playground.instructions") {
        const snapshot = await (0, instruction_bridge_1.loadInstructionBridge)(workspaceRoot);
