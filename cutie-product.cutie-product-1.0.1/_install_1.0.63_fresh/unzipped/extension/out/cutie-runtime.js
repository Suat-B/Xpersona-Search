...
function shouldUseObjectiveMode(goal, prompt, mentionContext, context) {
...
class CutieRuntime {
    ...
    async recoverActionableTurn(input) {
        ...
        const taskFrame = input.run.taskFrame ||
        ...
        if (isStructuredTooling(structured) &&
            ...
            return { response: structured, runPatch: recoveryTelemetryPatch };
        }
        ...
        if (shouldPushWorkspaceMutation && hasCompletedTool(input.run, "read_file")) {
            ...
            if (!analysis?.found && taskFrame.semanticQueries.length) {
    ...
    async runPrompt(input) {
        ...
        const preferredTarget = resolvePreferredTarget({
            prompt: input.prompt,
            mentionContext,
            context: initialContext,
            latestSessionTargetPath,
        });
        const preferredTargetPath = preferredTarget.path;
        const taskFrame = goal === "code_change"
            ? (0, cutie_code_intelligence_1.buildCodeTaskFrame)({
                prompt: input.prompt,
                mentionedPaths: mentionContext.mentionedPaths,
                preferredTargetPath,
                targetConfidence: preferredTarget.confidence,
            })
            : undefined;
        const targetCandidates = goal === "code_change"
            ? (0, cutie_code_intelligence_1.buildTargetCandidates)({
                preferredTargetPath,
                preferredTargetSource: preferredTarget.source,
                preferredTargetConfidence: preferredTarget.confidence,
        ...
        if (goal === "code_change" &&
            ...
            return this.failUntrustedCurrentFileRequest({
                ...
                modelTelemetry: createModelTelemetryPatch(modelDescription),
                callbacks: input.callbacks,
            });
        }
        ...
        try {
            mainLoop: while (true) {
                ...
                if (injectedPlanningTool) {
                    ...
                    ({ session, run } = await this.updateRun(session, run, createModelTelemetryPatch(turn)));
                ...
                if (structured.type === "final") {
                    ...
                    if (!objectiveCheck.ok) {
                        ...
                        const objectiveNeedsToolForcing = nextObjRepair >= 2;
                        const objectiveMustRecoverDirectly = nextObjRepair >= OBJECTIVE_NO_PROGRESS_REPAIR_CAP;
                ...
                for (const toolCall of batchToolCalls) {
                    ...
                    const noOpConclusion = toolResult.ok && toolCall.name === "run_command"
                    ...
                    ({ session, run } = await this.updateRun(session, run, {
                        ...
                        ...(verificationOutcome
                            ...
                            : {}),
                        ...(toolResult.ok && (0, cutie_policy_1.isWorkspaceMutationTool)(toolCall.name)
                            ? {
                    ...
                    if (toolResult.ok &&
                        ((0, cutie_policy_1.isWorkspaceMutationTool)(toolCall.name) ||
