import { validateWindowTarget } from "./cutie-policy";
import { CutieDesktopAdapter } from "./cutie-desktop-adapter";
import { CutieWorkspaceAdapter, CutieWorkspaceToolError } from "./cutie-workspace-adapter";
import type {
  CutieCheckpoint,
  CutieToolCall,
  CutieToolDomain,
...
type ToolDefinition = CutieProtocolToolDefinition;
...
function asString(value: unknown, fieldName: string, options?: { optional?: boolean }): string {
...
function asNumber(value: unknown, fieldName: string, options?: { min?: number; max?: number; optional?: boolean }): number {
...
function asLineEdits(value: unknown): Array<{ startLine: number; deleteLineCount: number; replacement: string }> {
...
function maybeNumber(value: unknown, min?: number, max?: number): number | undefined {
...
function getDefinition(name: CutieToolName): ToolDefinition {
...
function resultFromCheckpoint(checkpoint: CutieCheckpoint | null | undefined): Record<string, unknown> | undefined {
  return checkpoint ? { checkpoint } : undefined;
}

export class CutieToolRegistry {
  constructor(
    private readonly workspace: CutieWorkspaceAdapter,
    private readonly desktop: CutieDesktopAdapter
  ) {}
  ...
  describeToolsForPrompt(): string {
    return this.listDefinitions()
      .map(
        (tool) =>
          `- ${tool.name} [${tool.kind}/${tool.domain}]: ${tool.description} Schema: ${JSON.stringify(tool.inputSchema)}`
      )
      .join("\n");
  }
  ...
  async createAutomaticCheckpoint(reason: string): Promise<CutieToolResult> {
  ...
  async execute(toolCall: CutieToolCall, options?: { signal?: AbortSignal }): Promise<CutieToolResult> {
    ...
    try {
      ...
      switch (toolCall.name) {
        ...
        case "run_command": {
          const data = await this.workspace.runCommand({
            ...
            timeoutMs: maybeNumber(argumentsRecord.timeoutMs, 100, 300_000),
          });
          return {
            toolName: toolCall.name,
            kind: definition.kind,
        ...
        case "desktop_focus_window": {
          ...
          const validation = validateWindowTarget([windowId, title, app].filter(Boolean).join(" "));
          if (!validation.ok) {
            return {
              toolName: toolCall.name,
              kind: definition.kind,
        ...
        case "desktop_click": {
          ...
          const button = maybeString(argumentsRecord.button) as "left" | "right" | "middle" | undefined;
        ...
        case "desktop_wait": {
          const durationMs = asNumber(argumentsRecord.durationMs, "durationMs", { min: 0, max: 120_000 });
      ...
      const extraData =
        error instanceof CutieWorkspaceToolError && error.data && typeof error.data === "object"
          ? error.data
          : undefined;
      return {
        toolName: toolCall.name,
        ...
        ok: false,
        blocked:
          (error instanceof CutieWorkspaceToolError && error.blocked === true) ||
