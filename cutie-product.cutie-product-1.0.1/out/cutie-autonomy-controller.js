...
function normalizeToolPath(toolCall) {
    return (0, cutie_policy_1.normalizeWorkspaceRelativePath)(toolCall?.arguments && typeof toolCall.arguments.path === "string" ? String(toolCall.arguments.path) : null);
}
function isPreferredTargetInspectionBatch(run, batch) {
    const preferred = (0, cutie_policy_1.normalizeWorkspaceRelativePath)(run.preferredTargetPath || null);
    if (!preferred)
        return false;
    if (run.targetAcquisitionPhase !== "target_inspection" && run.currentRepairTactic !== "read_target")
        return false;
