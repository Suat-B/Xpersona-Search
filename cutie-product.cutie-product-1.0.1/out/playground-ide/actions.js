...
class ActionRunner {
    ...
    async captureUndoSnapshot(filePath) {
        ...
        if (batch.files.some((entry) => entry.path === filePath))
            return;
        const absolutePath = (0, pg_config_1.toAbsoluteWorkspacePath)(filePath);
        if (!absolutePath)
            return;
        try {
            batch.files.push({
    ...
    async apply(input) {
        ...
        const rootPath = (0, pg_config_1.getWorkspaceRootPath)();
        if (!rootPath) {
            return {
        ...
        for (const action of approvedActions) {
            if (action.type === "mkdir") {
                const absolutePath = (0, pg_config_1.toAbsoluteWorkspacePath)(action.path);
                if (!absolutePath) {
    ...
    async undoLastBatch() {
        ...
        for (const entry of [...this.undoBatch.files].reverse()) {
            const absolutePath = (0, pg_config_1.toAbsoluteWorkspacePath)(entry.path);
            if (!absolutePath)
                continue;
            await fs.mkdir(path.dirname(absolutePath), { recursive: true });
            if (!entry.existed) {
        ...
        for (const directory of [...this.undoBatch.createdDirectories].reverse()) {
            const absolutePath = (0, pg_config_1.toAbsoluteWorkspacePath)(directory);
            if (!absolutePath)
                continue;
    ...
    async buildValidationCommands(workspaceFolder, changedFiles) {
        ...
        const plans = changedFiles.map((filePath) => (0, validation_utils_1.planQuickValidationForFile)({
            filePath,
            absFile: path.join(workspaceFolder, (0, pg_config_1.normalizeWorkspaceRelativePath)(filePath) || filePath),
            workspaceFolder,
            changed: true,
