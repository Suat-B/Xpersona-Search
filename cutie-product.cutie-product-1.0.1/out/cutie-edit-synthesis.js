...
exports.buildEditIntent = buildEditIntent;
exports.synthesizeEditPlan = synthesizeEditPlan;
exports.realizeEditPlan = realizeEditPlan;
...
function resolveAnchorLine(lines, anchor) {
    ...
    lines.forEach((line, index) => {
        const hit = anchor.kind === "line_contains"
...
function appendCallArguments(line, argsToAdd) {
...
function applyEditOperation(lines, op) {
    ...
    switch (op.kind) {
        case "insert_before": {
            ...
            if (!block.trim())
                return { nextLines, applied: false, reason: `No insert block for ${op.description}.` };
            ...
            if (existingBlock === block)
                return { nextLines, applied: false };
        ...
        case "replace_value": {
            ...
            if (nextLine === nextLines[anchorIndex])
                return { nextLines, applied: false };
        ...
        default:
            return { nextLines, applied: false, reason: `Unsupported operation ${op.kind || "unknown"}.` };
    }
}
function isPineStrategyContent(content) {
    return /\/\/@version=\d+/i.test(content) && /\bstrategy\s*\(/.test(content);
}
function lastInputAnchor(content) {
...
function buildTrailingStopOperations(target) {
    const ops = [];
...
function buildTakeProfitOperations(target) {
    const ops = [];
    const inputAnchor = lastInputAnchor(target.content);
...
function buildRemoveIdentifierOperations(target, taskFrame) {
...
function buildEditIntent(input) {
    return {
...
function synthesizeEditPlan(input) {
    ...
    const confidence = planTargets.length === input.targets.length ? "high" : "medium";
    return {
        intent,
        plan: {
            targets: planTargets,
...
function derivePatchEdits(before, after) {
...
function realizeEditPlan(input) {
...
//# sourceMappingURL=cutie-edit-synthesis.js.map