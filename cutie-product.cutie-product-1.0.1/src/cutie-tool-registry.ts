...
const TOOL_DEFINITIONS: ToolDefinition[] = [
  ...
  },
  {
    ...
    inputShape:
      '{ "displayId": string, "normalizedX": number, "normalizedY": number, "button"?: "left" | "right" | "middle", "clickCount"?: number }',
  },
  {
    name: "desktop_type",
    kind: "mutate",
...
export class CutieToolRegistry {
  ...
  describeToolsForPrompt(): string {
    return this.listDefinitions()
      .map(
        (tool) =>
          `- ${tool.name} [${tool.kind}/${tool.domain}]: ${tool.description} Args: ${tool.inputShape}`
      )
      .join("\n");
  }
