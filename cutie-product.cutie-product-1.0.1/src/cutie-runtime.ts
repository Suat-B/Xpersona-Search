...
function tryNormalizeStructuredResponse(parsed: unknown): CutieStructuredResponse | null {
  ...
  if (record.type === "tool_call" && typeof toolCall.name === "string" && toolCall.arguments && typeof toolCall.arguments === "object") {
    return {
      type: "tool_call",
      tool_call: {
        name: toolCall.name as CutieToolCall["name"],
  ...
  if (!record.type && name && argumentsValue) {
    return {
      type: "tool_call",
      tool_call: {
        name: name as CutieToolCall["name"],
...
function salvageToolCallFromText(raw: string): CutieStructuredResponse | null {
  ...
  try {
    ...
    return {
      type: "tool_call",
      tool_call: {
        name: nameMatch[1] as CutieToolCall["name"],
...
function shouldSurfaceStreamingAssistantText(accumulated: string): boolean {
...
function createInitialRunState(sessionId: string): CutieRunState {
  return {
  ...
  async runPrompt(input: {
    ...
    try {
      while (true) {
        ...
        if (bootstrapToolCall) {
          ...
          const turn = await this.modelClient.streamTurn({
            ...
            onDelta: async (delta, accumulated) => {
              ...
              if (!shouldSurfaceStreamingAssistantText(accumulated)) return;
        ...
        if (!toolResult.ok) {
          if (toolResult.blocked && repeatedCallCount === CUTIE_MAX_IDENTICAL_CALLS) {
