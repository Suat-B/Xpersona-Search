...
function badge(label, value, tone = "cyan") {
...
function printChatBanner(config, mode, model, sessionId) {
...
async function readNonTtyInputLines() {
    const chunks = [];
...
async function createChatInputSource() {
...
function hasValidAccessToken(config) {
...
async function refreshBrowserAccessToken(config) {
...
async function resolveAuth(config) {
    ...
    if (apiKey)
        return { auth: { type: "apiKey", apiKey }, config };
    ...
    throw new Error("No CLI auth found. Run 'binary auth browser' (recommended) or 'binary auth set-key'.");
}
...
function getHostedModeNotice(mode) {
...
function encodePkceChallenge(verifier) {
    return createHash("sha256").update(verifier, "utf8").digest("base64url");
}
async function browserSignIn(config) {
    ...
    if (callback.state !== state)
        throw new Error("Browser auth state mismatch.");
...
function recoverPendingToolCallFromFinal(finalText, loopState, adapter) {
    const parsed = parseJsonCandidate(finalText);
    const toolCallRecord = parsed?.toolCall;
    if (!toolCallRecord || typeof toolCallRecord !== "object")
        return null;
...
function normalizeHostedEnvelope(envelope, fallback) {
    const normalized = {
        ...(fallback || {}),
        ...envelope,
    ...
    if (!normalized.pendingToolCall && normalized.final) {
        const recovered = recoverPendingToolCallFromFinal(normalized.final, normalized.loopState, normalized.adapter);
        if (recovered) {
...
async function streamPrompt(client, input, ui) {
    const envelope = {
        actions: [],
        final: "",
        missingRequirements: [],
    ...
    await client.assistStream(input, (event) => {
        ...
        if (ev === "run") {
            ...
            if (data.loopState && typeof data.loopState === "object") {
                envelope.loopState = data.loopState;
            }
            return;
        }
        if (ev === "actions") {
        ...
        if (ev === "tool_request") {
            if (event.data && typeof event.data === "object") {
                envelope.pendingToolCall = event.data;
            }
            return;
        }
        if (ev === "meta") {
    ...
    if (!normalizedEnvelope.pendingToolCall && normalizedEnvelope.final) {
        if (ui?.onFinal) {
            ...
            if (!printedToken)
                process.stdout.write(normalizedEnvelope.final);
            process.stdout.write("\n");
        }
    }
...
async function runHostedPrompt(client, input, workspaceRoot, ui) {
...
async function runLocalHostPrompt(hostClient, input, workspaceRoot, ui) {
    const envelope = {
        actions: [],
        final: "",
        missingRequirements: [],
    ...
    await hostClient.assistStream({
        ...
        if (ev === "tool_result") {
            ...
            if (ui?.onToolResultEvent)
                ui.onToolResultEvent(name, summary, ok);
            else
    ...
    if (envelope.final) {
        if (ui?.onFinal) {
            ...
            if (!printedToken)
                process.stdout.write(envelope.final);
            process.stdout.write("\n");
        }
    }
    return envelope;
...
async function createDebugWorkspace(parsed) {
...
async function handleDebugRuntime(parsed, config) {
    const task = parsed.positionals.slice(1).join(" ").trim();
    if (!task)
        throw new Error("Usage: binary debug-runtime \"<task>\"");
    ...
    const warnings = [];
    const reportPath = path.resolve(getFlagString(parsed, "out") ??
    ...
    if (hostedModeNotice)
        warnings.push(hostedModeNotice);
    console.log(color("Binary IDE Runtime Debug", "cyan"));
    console.log(dim(rule()));
    console.log(`Task: ${task}`);
    console.log(`Mode: ${mode}`);
    if (hostedMode !== mode)
        console.log(`Hosted mode: ${hostedMode}`);
...
async function handleAuth(parsed, config) {
    ...
    if (sub === "browser" || sub === "login") {
        ...
        console.log(`Browser sign-in complete${next.browserAuth?.email ? ` as ${next.browserAuth.email}` : ""}.`);
        return;
    }
    if (sub === "clear") {
        const refreshToken = getBrowserRefreshToken(config);
...
async function handleConfig(parsed, config) {
    ...
    if (sub === "set-base-url") {
        ...
        if (!url)
            throw new Error("Usage: binary config set-base-url <url>");
    ...
    if (sub === "set-local-host-url") {
        const url = parsed.positionals[2];
        if (!url)
            throw new Error("Usage: binary config set-local-host-url <url>");
    ...
    if (sub === "set-model") {
        ...
        if (!model)
            throw new Error("Usage: binary config set-model <model>");
...
async function handleChat(parsed, config) {
    ...
    if (transport.selected === "host" && transport.hostClient) {
        ...
        while (true) {
            ...
            if (line.startsWith("/mode ")) {
                ...
                if (nextModeNotice)
                    console.log(dim(`mode note -> ${nextModeNotice}`));
                continue;
            }
            if (line === "/clear") {
                clearIfTty();
            ...
            const result = await runLocalHostPrompt(transport.hostClient, {
                task: line,
                mode: activeMode,
                model,
    ...
    const client = new PlaygroundClient({
        ...
        auth: resolvedAuth.auth.type === "apiKey"
    ...
    while (true) {
        ...
        if (line === "/new") {
            ...
            activeConfig = resolvedAuth.config;
            client.setAuth(resolvedAuth.auth.type === "apiKey"
                ? { apiKey: resolvedAuth.auth.apiKey }
                : { bearer: resolvedAuth.auth.accessToken });
            try {
...
async function handleRuns(parsed, config) {
    ...
    if (sub === "show") {
        ...
        if (!runId)
            throw new Error("Usage: binary runs show <runId>");
    ...
    if (sub === "tail") {
        const runId = parsed.positionals[2];
        if (!runId)
            throw new Error("Usage: binary runs tail <runId>");
    ...
    if (sub === "export") {
        const runId = parsed.positionals[2];
        if (!runId)
            throw new Error("Usage: binary runs export <runId> [--output run.json]");
...
async function handleSessions(parsed, config) {
    ...
    if (sub === "show") {
        ...
        if (!sessionId)
            throw new Error("Usage: binary sessions show <sessionId>");
...
async function handleReplay(parsed, config) {
    ...
    if (!sessionId)
        throw new Error("Usage: binary replay <sessionId> [--mode plan]");
...
async function handleExecute(parsed, config) {
    ...
    if (!filePath)
        throw new Error("Usage: binary execute --file actions.json [--session <id>]");
...
async function handleIndex(parsed, config) {
    ...
    if (sub === "query") {
        ...
        if (!question)
            throw new Error("Usage: binary index query --project <key> \"<question>\"");
        const limitRaw = getFlagString(parsed, "limit", "l");
...
async function run() {
    ...
    if (command === "set-model") {
        ...
        await handleConfig({ positionals: ["config", "set-model", ...(modelArg ? [modelArg] : [])], flags: parsed.flags }, config);
        return;
    }
