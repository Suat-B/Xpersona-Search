...
function detectFormatFromBytes(buffer, targetPath) {
...
async function readPrefix(targetPath, limit) {
    const handle = await fs.open(targetPath, "r");
    try {
...
function toBinaryMutationPolicy(descriptor, approved) {
...
export function classifyBinaryRisk(targetPath, isExecutable, artifactKind) {
    if (isRawDevicePath(targetPath) || isProtectedSystemPath(targetPath))
        return "critical";
    if (artifactKind === "disk_image" || artifactKind === "firmware" || artifactKind === "system_file")
        return "critical";
    if (isExecutable || artifactKind === "executable" || artifactKind === "shared_library")
        return "high";
    return "low";
}
export async function resolveBinaryTarget(workspaceRoot, inputPath) {
    ...
    const scope = isUnderPath(resolvedWorkspaceRoot, absolutePath) ? "workspace" : "machine";
    const displayPath = scope === "workspace" ? normalizeDisplayPath(path.relative(resolvedWorkspaceRoot, absolutePath)) : absolutePath;
    return {
        inputPath: raw,
        absolutePath,
...
async function detectExecutable(targetPath, stats) {
    const extension = path.extname(targetPath).toLowerCase();
...
export async function describeBinaryTarget(workspaceRoot, inputPath) {
    ...
    try {
        ...
        const riskClass = isProtectedSystemPath(resolved.absolutePath) || artifactKind === "disk_image" || artifactKind === "firmware"
            ? "critical"
            : isExecutable
                ? "high"
                : "low";
...
export async function isLikelyBinaryFile(workspaceRoot, inputPath) {
    const descriptor = await describeBinaryTarget(workspaceRoot, inputPath);
    if (!descriptor || !descriptor.exists || !descriptor.isRegularFile)
        return looksLikeBinaryPath(inputPath);
    if (descriptor.artifactKind !== "regular_file" && descriptor.artifactKind !== "document")
        return true;
...
export function binaryTextToolFailure(pathValue) {
    return {
        blocked: true,
...
export async function readBinaryChunk(workspaceRoot, inputPath, offsetInput, lengthInput) {
    const descriptor = await describeBinaryTarget(workspaceRoot, inputPath);
    if (!descriptor || !descriptor.exists || !descriptor.isRegularFile || descriptor.size == null)
        return null;
...
function parseSearchPattern(pattern, encodingInput) {
...
function findPatternOffsets(haystack, needle, limit) {
    if (!needle.length)
        return [];
    const out = [];
    let offset = 0;
...
export async function searchBinary(workspaceRoot, inputPath, pattern, options) {
    const descriptor = await describeBinaryTarget(workspaceRoot, inputPath);
    if (!descriptor || !descriptor.exists || !descriptor.isRegularFile || descriptor.size == null)
        return null;
...
export async function analyzeBinary(workspaceRoot, inputPath) {
    const descriptor = await describeBinaryTarget(workspaceRoot, inputPath);
    if (!descriptor || !descriptor.exists || !descriptor.isRegularFile)
        return null;
...
export async function hashBinary(workspaceRoot, inputPath) {
    return await describeBinaryTarget(workspaceRoot, inputPath);
}
async function ensureBinarySnapshot(targetPath) {
    const directory = binarySnapshotsDir();
    await fs.mkdir(directory, { recursive: true });
    const snapshotPath = path.join(directory, `${new Date().toISOString().replace(/[:.]/g, "-")}-${randomUUID()}-${path.basename(targetPath)}`);
...
export async function writeBinaryFile(workspaceRoot, inputPath, args) {
    const resolved = await resolveBinaryTarget(workspaceRoot, inputPath);
    if (!resolved)
    ...
    if (policy.blocked)
        return { descriptor: synthesizedDescriptor, policy };
...
function applyPatchOperations(before, operations) {
...
export async function patchBinary(workspaceRoot, inputPath, args) {
    const descriptor = await describeBinaryTarget(workspaceRoot, inputPath);
