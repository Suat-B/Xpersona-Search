...
import { analyzeBinary, binaryTextToolFailure, hashBinary, isLikelyBinaryFile, looksLikeBinaryPath, patchBinary, readBinaryChunk, searchBinary, writeBinaryFile, } from "./binary-access.js";
...
async function collectWorkspaceFiles(root, limit) {
    const out = [];
    const stack = [root];
...
export class CliToolExecutor {
    ...
    async resolveWithObservedRoot(inputPath, options) {
    ...
    inferCommandCwd(command, explicitCwd) {
    ...
    resolveStaticPath(inputPath, options) {
        const direct = resolveWorkspacePath(this.workspaceRoot, String(inputPath || ""));
        if (!direct)
            return null;
        ...
        try {
            ...
            if (toolCall.name === "read_file") {
                ...
                try {
                    ...
                    if (error?.code === "ENOENT" && suggestion) {
                        return this.fail(toolCall.id, toolCall.name, `Missing file ${resolved.relativePath}. Did you mean ${suggestion}?`);
                    }
                    throw error;
                }
                const lines = raw.replace(/\r\n/g, "\n").split("\n");
            ...
            if (toolCall.name === "mkdir") {
                const resolved = (await this.maybeRewriteIntoObservedRoot(String(args.path || ""))) ||
            ...
            if (toolCall.name === "write_file") {
                ...
                if (looksLikeBinaryPath(resolved.relativePath) ||
            ...
            if (toolCall.name === "edit") {
                ...
                if (before == null) {
                    const suggestion = await suggestWorkspacePath(this.workspaceRoot, resolved.relativePath);
                    return this.fail(toolCall.id, toolCall.name, suggestion
                ...
                if (applied.status !== "applied" || typeof applied.content !== "string") {
                    return this.fail(toolCall.id, toolCall.name, applied.reason || `Patch failed for ${resolved.relativePath}.`);
                }
            ...
            if (toolCall.name === "get_workspace_memory" || toolCall.name === "get_diagnostics") {
                return {
                    ...
                    name: toolCall.name,
                    ok: true,
                    summary: toolCall.name === "get_workspace_memory"
            ...
            if (toolCall.name === "stat_binary" || toolCall.name === "hash_binary") {
                ...
                return {
                    ...
                    name: toolCall.name,
                    ok: true,
                    summary: toolCall.name === "hash_binary"
            ...
            if (toolCall.name === "read_binary_chunk") {
                const chunk = await readBinaryChunk(this.workspaceRoot, String(args.path || ""), Number(args.offset || 0), Number(args.length || 4096));
                if (!chunk) {
            ...
            return this.fail(toolCall.id, toolCall.name, error instanceof Error ? error.message : String(error));
        }
    }
    fail(toolCallId, name, message) {
        return {
            toolCallId,
            name,
            ok: false,
