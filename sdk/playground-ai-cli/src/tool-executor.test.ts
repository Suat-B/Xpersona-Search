...
import type { PendingToolCall, PlaygroundToolName } from "./types.js";
...
describe("inferTaskProjectRoot", () => {
  it("extracts project folder names from natural language tasks", () => {
    expect(inferTaskProjectRoot("Create a new project folder named duration-toolkit")).toBe(
      "duration-toolkit"
    );
    expect(inferTaskProjectRoot("Add a folder named api-core in the current workspace")).toBe(
      "api-core"
    );
...
describe("CliToolExecutor", () => {
  ...
  it("reads a file range from the workspace", async () => {
    ...
    const result = await executor.execute(
      toolCall("read_file", { path: "src/hello.ts", startLine: 2, endLine: 3 })
    );
  ...
  it("searches the workspace with ripgrep", async () => {
    ...
    const executor = new CliToolExecutor(workspace);
    const result = await executor.execute(
      toolCall("search_workspace", { query: "needle-value", limit: 2 })
    );
  ...
  it("tracks an observed root and rewrites new paths into that project", async () => {
    ...
    const writeResult = await executor.execute(
      toolCall("write_file", { path: "src/index.ts", content: "export const value = 1;\n" })
    );

    const written = await fs.readFile(
      path.join(workspace, "duration-toolkit", "src", "index.ts"),
      "utf8"
    );
  ...
  it("infers command cwd from the preferred project root and explicit cwd", async () => {
    ...
    const defaultCwdResult = await executor.execute(
      toolCall("run_command", {
        command: 'node -e "process.stdout.write(process.cwd())"',
      })
    );
    const explicitCwdResult = await executor.execute(
      toolCall("run_command", {
        command: 'node -e "process.stdout.write(process.cwd())"',
        cwd: "src",
      })
  ...
  it("returns placeholder results for checkpoint, memory, and diagnostics tools", async () => {
    ...
    const checkpoint = await executor.execute(
      toolCall("create_checkpoint", { reason: "Before edits" })
    );
  ...
  it("rejects unsupported tools", async () => {
    ...
    const result = await executor.execute(toolCall("teleport_file" as PlaygroundToolName));
  ...
  it("reads and analyzes binary chunks", async () => {
    ...
    const executor = new CliToolExecutor(workspace);
    const chunkResult = await executor.execute(
      toolCall("read_binary_chunk", { path: "fixtures/sample.bin", offset: 0, length: 6 })
    );
    const analysisResult = await executor.execute(
      toolCall("analyze_binary", { path: "fixtures/sample.bin" })
    );
