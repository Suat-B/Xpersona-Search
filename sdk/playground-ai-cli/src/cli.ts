...
function requireApiKey(config: CliConfig): string {
  ...
  if (!key) {
    throw new Error(
      "No API key configured. Run 'playground auth set-key' or set PLAYGROUND_AI_API_KEY."
    );
  }
  return key;
}
