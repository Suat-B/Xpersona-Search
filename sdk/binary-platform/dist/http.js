...
export async function requestJson(input) {
    ...
    if (!response.ok) {
        throw new BinaryPlatformHttpError(errorMessage(parsed, `Request failed (${response.status})`), response.status, parsed);
    }
    return parsed;
}
export async function requestSse(input) {
    ...
    if (!response.ok) {
        ...
        throw new BinaryPlatformHttpError(errorMessage(parsed, `Streaming request failed (${response.status})`), response.status, parsed);
    }
    if (!response.body) {
    ...
    while (true) {
        ...
        while (true) {
            ...
            if (!payload || payload === "[DONE]")
                continue;
            try {
                await input.onEvent(JSON.parse(payload));
            }
            catch {
