...
export async function sendAccountClaimEmail(to: string, claimUrl: string): Promise<void> {
  const from = getFrom();
  const subject = "Claim your Xpersona account";
  const safeUrl = claimUrl.trim();
...
<body style="margin:0;padding:0;font-family:system-ui,-apple-system,sans-serif;background:#0a0a0a;color:#e5e5e5;">
  <div style="max-width:480px;margin:32px auto;padding:24px;">
    <h1 style="font-size:20px;margin:0 0 16px;">Claim your account</h1>
    <p style="margin:0 0 24px;line-height:1.5;color:#a3a3a3;">
      You entered this email during checkout. Click the button below to verify it and convert your temporary account into a permanent one.
    </p>
    <a href="${safeUrl}" style="display:inline-block;background:#30d158;color:#0a0a0a;text-decoration:none;padding:12px 24px;border-radius:12px;font-weight:700;">
      Verify email
    </a>
    <p style="margin:24px 0 0;font-size:12px;color:#737373;">
...
`.trim();
  ...
  const text = `Claim your Xpersona account\n\nClick the link below to verify your email and convert your account:\n\n${safeUrl}\n\nIf you didn't make this purchase, you can ignore this email.`;
  ...
  // Fallback: log in development
  if (process.env.NODE_ENV === "development") {
    console.log("[email] Account claim (no RESEND_API_KEY):");
    console.log("  To:", to);
    console.log("  Claim URL:", safeUrl);
    return;
  }
