...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      limits: {
        contextHardCap: limits.contextHardCap,
        maxInputTokensPerRequest: limits.maxInputTokensPerRequest,
        maxOutputTokens: limits.maxOutputTokens,
        maxRequestsPerCycle: limits.maxRequestsPerCycle,
        maxTotalTokensPerCycle: limits.maxTotalTokensPerCycle,
        maxTotalTokensPerMonth: limits.maxTotalTokensPerMonth,
      },
      cycle: {
        requestsUsed: stats.cycle.requestsUsed,
        requestsRemaining: Math.max(0, limits.maxRequestsPerCycle - stats.cycle.requestsUsed),
        requestsLimit: stats.cycle.requestsLimit,
        tokensTotalUsed: stats.cycle.tokensTotalUsed,
        tokensTotalRemaining: Math.max(0, limits.maxTotalTokensPerCycle - stats.cycle.tokensTotalUsed),
        tokensTotalLimit: stats.cycle.tokensTotalLimit,
        startsAt: stats.cycle.startsAt,
        endsAt: stats.cycle.endsAt,
      },
      thisMonth: {
        tokensInput: stats.thisMonth.tokensInput,
        tokensOutput: stats.thisMonth.tokensOutput,
        tokensTotal: stats.thisMonth.tokensTotal,
        tokensRemaining: Math.max(0, limits.maxTotalTokensPerMonth - stats.thisMonth.tokensTotal),
