...
export function DepositPageClient({ initialBalance }: DepositPageClientProps) {
  const searchParams = useSearchParams();
  const success = searchParams.get("success") === "1";

  const [balance, setBalance] = useState<number | null>(
    initialBalance !== undefined && initialBalance !== null ? initialBalance : null
  );
  ...
  const loadBalance = useCallback(async (silent = false): Promise<boolean> => {
  ...
  useEffect(() => {
    ...
    const hasServerBalance = initialBalance !== undefined && initialBalance !== null;
    ...
    if (hasServerBalance) {
      const refreshInBackground = () => loadBalance(true);
