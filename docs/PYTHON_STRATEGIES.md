...
The dice round API expects `amount`, `target`, and `condition` ("over" / "under"). Your `BetDecision` maps 1:1 to `POST /api/v1/games/dice/round`. Limits and house edge are available via `ctx.get_limits()`.
