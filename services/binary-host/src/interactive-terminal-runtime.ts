...
function buildDefaultShellSpec(shell?: string): ShellLaunchSpec {
...
export class InteractiveTerminalRuntime {
  ...
  async sendInput(input: SendInput): Promise<{
    session: InteractiveTerminalSessionView;
    output: string;
    truncated: boolean;
  }> {
  ...
  async readOutput(input: ReadSessionInput): Promise<{
    session: InteractiveTerminalSessionView;
    output: string;
    truncated: boolean;
  }> {
    ...
    const afterCursor =
      typeof input.afterCursor === "number" && Number.isFinite(input.afterCursor)
  ...
  async terminateSession(sessionId: string): Promise<InteractiveTerminalSessionView> {
  ...
  private appendChunk(session: InternalTerminalSession, chunk: string): void {
  ...
  private currentCursor(session: InternalTerminalSession): number {
  ...
  private sliceBuffer(
    ...
    afterCursor: number,
    maxChars: number
  ): { output: string; cursor: number; truncated: boolean } {
  ...
  private async waitForQuiet(
    ...
    while (Date.now() - startedAt < timeoutMs) {
      ...
      if (currentCursor > afterCursor) {
        if (idleMs >= waitForMs) return;
      } else if (Date.now() - startedAt >= waitForMs) {
        return;
      }
      if (session.status !== "running" && idleMs >= 40) return;
      await delay(40);
    }
  }

  private requireSession(sessionId: string): InternalTerminalSession {
  ...
  private toView(session: InternalTerminalSession): InteractiveTerminalSessionView {
    return {
      sessionId: session.id,
      cwd: session.cwd,
      shell: session.shell,
