...
type MockCommandRunner = (command: string, args: string[], cwd: string) => Promise<{
  ok: boolean;
  stdout: string;
  stderr: string;
...
async function createRepoService(commandRunner?: MockCommandRunner): Promise<{ service: RepoModelService; root: string }> {
...
describe("RepoModelService", () => {
  it("builds a local repo summary with symbols, hotspots, and validation hints", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "demo", scripts: { test: "node --test", lint: "eslint ." } }, null, 2),
      "utf8"
    );
  ...
  it("uses ast-grep engine for symbol and reference lookup when available", async () => {
    const commandRunner: MockCommandRunner = async (command, args) => {
  ...
  it("uses tree-sitter fallback when ast-grep is unavailable", async () => {
    const commandRunner: MockCommandRunner = async () => {
      return { ok: false, stdout: "", stderr: "missing", exitCode: 1 };
    };
    ...
    await fs.writeFile(
      ...
      ["import { greet } from './person';", "export const message = greet('Ada');", ""].join("\n"),
      "utf8"
    );
