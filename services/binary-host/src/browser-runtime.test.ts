...
import {
  ...
  buildBrowserSiteSearchUrl,
  inferBrowserMissionUrlFromQuery,
  rankBrowserResultCandidates,
...
describe("BrowserRuntimeController selector inference", () => {
  ...
  it("builds direct site search routes for supported domains", () => {
    expect(buildBrowserSiteSearchUrl("https://www.youtube.com/", "outdoor boys")).toBe(
      "https://www.youtube.com/results?search_query=outdoor%20boys"
    );
    expect(buildBrowserSiteSearchUrl("https://github.com/", "browser mission")).toBe(
      "https://github.com/search?q=browser%20mission"
    );
  });
  ...
  it("keeps mission final page pinned even if active-page state drifts elsewhere", async () => {
    ...
    controller.resolveMissionResultPage = async (_policy: unknown, pageId: string, fallback: unknown) =>
      pageId === "page_search"
        ? {
            id: "page_search",
            title: "Outdoor Boys",
  ...
  it("surfaces the active browser mission lease in collected context", async () => {
    ...
    expect(context.activeMissionLease).toEqual(
      expect.objectContaining({
        leaseId: "lease_1",
        missionKind: "browser_complete_form",
        pageId: "page_1",
  ...
  it("allows assertPageTarget when origin and lease match", async () => {
    ...
    await expect(
      controller.assertPageTarget(policy, {
        pageId: "page_1",
