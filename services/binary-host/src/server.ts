...
function inferBrowserMissionQuery(task: string): string {
  ...
  if (!source) return "";
  const searchMatch =
    source.match(/\bsearch\s+(?:for\s+)?(.+?)(?:\s+(?:and|then)\s+(?:open|click)\b|$)/i) ||
...
function buildGatewayExecutionHints(input: {
  ...
  const adapterMode: BinaryAdapterMode = smallModelForced && !browserTask ? "force_binary_tool_adapter" : "auto";
    ...
    try {
      ...
      if (directTargetQuery && canOpenFiles) {
        ...
        if (directFolderPath) {
          ...
          await emitHostStatus(
            run,
            `Binary opened ${path.basename(directFolderPath) || directFolderPath} directly from the local machine lane.`,
            attachedRes,
            {
              shortcutKind: "open_folder",
      ...
      if (parsedAction?.kind === "launch_app" && canOpenApps) {
        ...
        if (folderIntent && canOpenFiles) {
          ...
          if (folderPath) {
            ...
            await emitHostStatus(
              run,
              `Binary opened ${path.basename(folderPath) || folderPath} directly from the local machine lane.`,
              attachedRes,
              {
                shortcutKind: "open_folder",
                targetPath: folderPath,
...
async function getOpenHandsRuntimeHealth(task?: string): Promise<OpenHandsRuntimeStatus> {
...
async function executeHostedAgentProbeTurn(input: {
  ...
  if (localGatewayModelCandidate) {
    const gatewayBaseUrl =
      executionDecision.lane === "openhands_remote" && remoteHealth?.available && remoteHealth.gatewayUrl
...
function applyEnvelopeToRun(run: StoredHostRun, envelope: AssistRunEnvelope): void {
  ...
  if (envelope.executionLane) run.executionLane = envelope.executionLane;
...
function buildFirstResponseProgressMessage(tick: number): string {
  if (tick <= 0) return "Thinking";
  if (tick === 1) return "Thinking. Preparing the first actionable step now.";
  if (tick % 2 === 0) return "Thinking. Checking the next tool action.";
...
async function executeHostRun(runId: string, attachedRes?: ServerResponse | null): Promise<void> {
  ...
  try {
    ...
    if (assistTransport === "local_gateway") {
      const gatewayBaseUrl =
        executionConfig.executionLane === "openhands_remote" && remoteHealth?.available && remoteHealth.gatewayUrl
          ? remoteHealth.gatewayUrl
          : undefined;
    ...
    const requiresExecutionProof =
      Boolean(run.workspaceRoot) &&
      !runLikelyDesktopTask(run) &&
