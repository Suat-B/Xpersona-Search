    ...
    async execute(pendingToolCall) {
        ...
        try {
            ...
            if (toolCall.name === "browser_focus_page") {
                ...
                if (!pageId)
                    return fail(toolCall, "browser_focus_page requires pageId.");
            ...
            if (toolCall.name === "browser_snapshot_dom") {
                const pageId = String(args.pageId || "").trim();
                if (!pageId)
                    return fail(toolCall, "browser_snapshot_dom requires pageId.");
            ...
            if (toolCall.name === "browser_query_elements") {
                const pageId = String(args.pageId || "").trim();
                if (!pageId)
                    return fail(toolCall, "browser_query_elements requires pageId.");
            ...
            if (toolCall.name === "browser_click") {
                const pageId = String(args.pageId || "").trim();
                if (!pageId)
                    return fail(toolCall, "browser_click requires pageId.");
            ...
            if (toolCall.name === "browser_press_keys") {
                ...
                if (!pageId || !keys.length)
                    return fail(toolCall, "browser_press_keys requires pageId and keys.");
            ...
            if (toolCall.name === "browser_scroll") {
                const pageId = String(args.pageId || "").trim();
                if (!pageId)
                    return fail(toolCall, "browser_scroll requires pageId.");
            ...
            if (toolCall.name === "browser_wait_for") {
                const pageId = String(args.pageId || "").trim();
                if (!pageId)
                    return fail(toolCall, "browser_wait_for requires pageId.");
            ...
            if (toolCall.name === "browser_read_text") {
                const pageId = String(args.pageId || "").trim();
                if (!pageId)
                    return fail(toolCall, "browser_read_text requires pageId.");
            ...
            if (toolCall.name === "browser_read_form_state") {
                const pageId = String(args.pageId || "").trim();
                if (!pageId)
                    return fail(toolCall, "browser_read_form_state requires pageId.");
            ...
            if (toolCall.name === "browser_capture_page") {
                const pageId = String(args.pageId || "").trim();
                if (!pageId)
                    return fail(toolCall, "browser_capture_page requires pageId.");
            ...
            if (toolCall.name === "browser_get_network_activity") {
                const pageId = String(args.pageId || "").trim();
                if (!pageId)
                    return fail(toolCall, "browser_get_network_activity requires pageId.");
            ...
            if (toolCall.name === "browser_get_console_messages") {
                const pageId = String(args.pageId || "").trim();
                if (!pageId)
                    return fail(toolCall, "browser_get_console_messages requires pageId.");
