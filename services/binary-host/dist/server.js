...
import { continueHostedRun, runHostedAgentProbe, submitHostedUserInput, streamHostedAssist, } from "./hosted-transport.js";
...
import { MachineAutonomyController, defaultMachineAutonomyPolicy, parseMachineAutonomyTask, } from "./machine-autonomy.js";
...
import { getRemoteRuntimeHealth, resolveExecutionLane, resolveOpenHandsPluginPacks, resolveOpenHandsSkillSources, shouldEnableSampledTracing, } from "./openhands-capabilities.js";
import { buildConnectionView, buildOpenHandsMcpConfig, connectionHasRequiredSecret, getConnectionStatus, importConnectionsFromMcpJson, validateConnectionDraft, } from "./connections.js";
...
const DEFAULT_HOSTED_DELEGATION_CONFIG = {
    enabled: true,
    mode: "auto",
    maxChildren: 3,
...
const FAST_FIRST_TURN_BUDGET_MS = 30_000;
...
const FAST_FIRST_TURN_STALL_TIMEOUT_MS = 10_000;
...
function buildTerminalState(run, pendingToolCall, toolResult) {
    ...
    const sessionRecord = toolResult?.data && typeof toolResult.data === "object" && toolResult.data.session && typeof toolResult.data.session === "object"
        ? toolResult.data.session
        : null;
    ...
    const cwd = typeof sessionRecord?.cwd === "string"
    ...
    return {
        sessionId: typeof sessionRecord?.sessionId === "string"
        ...
        terminalProof: typeof toolResult?.data?.output === "string" && toolResult.data.output.trim()
...
function buildWorldSummaryText(summary) {
    ...
    if (machineRoot)
        parts.push(`machine_home=${machineRoot}`);
    if (focusedRepo)
        parts.push(`focus_repo=${focusedRepo}`);
    if (focusedWorkspace)
        parts.push(`focus_workspace=${focusedWorkspace}`);
    if (activeWorkspace)
...
async function buildRepoContextSlice(workspaceRoot, task) {
    if (!workspaceRoot)
        return undefined;
    return (await repoModelService.getSummary(workspaceRoot, task));
}
async function buildVerificationPlanSlice(workspaceRoot, paths) {
    if (!workspaceRoot)
        return undefined;
    return (await repoModelService.getValidationPlan(workspaceRoot, { paths }));
}
function normalizeSpeedProfile(value) {
    return value === "balanced" || value === "thorough" ? value : "fast";
}
function isExplicitMachineShortcutTask(task) {
    ...
    if (extractInteractiveTerminalShortcutPlan(task))
        return true;
    if (looksLikeWebAutomationIntent(task))
        return false;
    if (extractOpenFolderQuery(task) || extractDirectOpenTarget(task))
        return true;
    const parsedAction = parseMachineAutonomyTask(task);
    return parsedAction?.kind === "launch_app";
}
function classifyTaskSpeed(task, workspaceRoot) {
    const normalized = String(task || "").trim().toLowerCase();
    if (!normalized)
        return "chat_only";
    ...
    if (deepCodeSignals.some((signal) => normalized.includes(signal)))
        return "deep_code";
    if (isExplicitMachineShortcutTask(task))
    ...
    if ((directActionPattern.test(normalized) || politeActionPatterns.some((pattern) => pattern.test(normalized))) &&
    ...
    if (normalized.endsWith("?") || chatOnlySignals.some((signal) => normalized.startsWith(signal)))
        return "chat_only";
...
function selectInitialWorldContextTier(taskSpeedClass) {
    if (taskSpeedClass === "deep_code")
        return "full";
    if (taskSpeedClass === "tool_heavy")
        return "standard";
    return "minimal";
}
function extractWorldRouteDiagnostics(worldContext) {
    const topRoute = worldContext && Array.isArray(worldContext.routeRecommendations) && worldContext.routeRecommendations[0] && typeof worldContext.routeRecommendations[0] === "object"
        ? worldContext.routeRecommendations[0]
        : undefined;
    ...
    const contradictory = attentionQueue.some((item) => item && typeof item === "object" && item.kind === "contradiction");
    const stale = attentionQueue.some((item) => item && typeof item === "object" && item.kind === "stale_belief");
    const blockedGoal = attentionQueue.some((item) => item && typeof item === "object" && item.kind === "blocked_goal");
...
function selectNextWorldContextTier(input) {
    ...
    if (input.currentTier === "full")
        return "full";
    if (input.taskSpeedClass === "deep_code")
        return "full";
...
function shouldDeferRepoContext(taskSpeedClass) {
    return taskSpeedClass !== "deep_code";
}
function shouldDeferVerificationPlan(taskSpeedClass) {
    return taskSpeedClass !== "deep_code";
}
function inferReasoningEffort(speedProfile, taskSpeedClass, candidate, initialTurn) {
    if (speedProfile === "thorough")
        return "high";
...
function latencyTierWeight(value) {
    switch (value) {
        case "fast":
            return 3;
        case "balanced":
...
function intendedUseWeight(value, taskSpeedClass, initialTurn) {
    if (taskSpeedClass === "chat_only")
        return value === "chat" ? 3 : value === "action" ? 1 : 0;
    if (taskSpeedClass === "deep_code")
        return value === "repair" ? 3 : value === "action" ? 2 : 1;
    if (!initialTurn)
        return value === "action" ? 3 : value === "repair" ? 2 : 1;
...
function rankModelCandidateForStartup(input) {
    ...
    const explicitMatch = normalizedRequested &&
...
function resolveTurnBudgetMs(speedProfile, taskSpeedClass) {
    if (speedProfile === "fast") {
        if (taskSpeedClass === "chat_only" || taskSpeedClass === "simple_action")
            return FAST_TURN_BUDGET_MS;
        if (taskSpeedClass === "tool_heavy")
            return BALANCED_TURN_BUDGET_MS;
    }
    if (speedProfile === "balanced")
        return BALANCED_TURN_BUDGET_MS;
    return THOROUGH_TURN_BUDGET_MS;
}
function resolveTurnMaxIterations(speedProfile, taskSpeedClass) {
    if (speedProfile === "fast") {
        if (taskSpeedClass === "chat_only" || taskSpeedClass === "simple_action")
            return FAST_TURN_MAX_ITERATIONS;
        if (taskSpeedClass === "tool_heavy")
            return BALANCED_TURN_MAX_ITERATIONS;
    }
    if (speedProfile === "balanced")
        return BALANCED_TURN_MAX_ITERATIONS;
    return THOROUGH_TURN_MAX_ITERATIONS;
}
function resolveTurnStallTimeoutMs(speedProfile, taskSpeedClass) {
    if (speedProfile === "fast") {
        if (taskSpeedClass === "chat_only" || taskSpeedClass === "simple_action")
            return FAST_STALL_TIMEOUT_MS;
        if (taskSpeedClass === "tool_heavy")
            return BALANCED_STALL_TIMEOUT_MS;
    }
    if (speedProfile === "balanced")
        return BALANCED_STALL_TIMEOUT_MS;
...
function matchPattern(value, pattern) {
    ...
    if (wildcard)
        return wildcard.test(left);
    return left.includes(right);
}
function candidateMatchesSmallModelPattern(candidate, rawPattern) {
    ...
    if (!scopedPattern)
        return false;
    if (scoped === "alias")
        return matchPattern(candidate.alias, scopedPattern);
    if (scoped === "model")
        return matchPattern(candidate.model, scopedPattern);
    if (scoped === "provider")
        return matchPattern(candidate.provider, scopedPattern);
    if (scoped === "display")
        return matchPattern(candidate.displayName, scopedPattern);
    const targets = [
        candidate.alias,
...
function isSmallModelForcedCandidate(candidate, policy) {
    if (!candidate)
        return false;
    if (policy.mode !== "force_binary_tool_adapter")
        return false;
...
function buildTurnRoutePolicy(input) {
    const requestPolicy = input.request.routePolicy && typeof input.request.routePolicy === "object" ? input.request.routePolicy : {};
    const override = input.override || {};
    const turnBudgetMs = toFinitePositiveNumber(override.turnBudgetMs) ||
...
function tightenFirstTurnRoutePolicy(routePolicy, taskSpeedClass) {
    if (taskSpeedClass !== "chat_only" && taskSpeedClass !== "simple_action") {
        return routePolicy;
    }
    return {
...
function buildGatewayRuntimeModelCandidate(runtimeStatus, fallbackApiKey) {
    const raw = runtimeStatus?.currentModelCandidate && typeof runtimeStatus.currentModelCandidate === "object"
        ? runtimeStatus.currentModelCandidate
        : null;
    if (!raw)
        return null;
    ...
    if (!alias || !model || !provider || !baseUrl)
        return null;
    return {
        alias,
        provider,
        ...
        ...(Array.isArray(raw.modelFamilies)
            ? {
                modelFamilies: raw.modelFamilies.filter((value) => typeof value === "string" && Boolean(value.trim())),
            }
            : {}),
...
function inferBrowserMissionQuery(task) {
...
function looksLikeWebAutomationIntent(task) {
    ...
    if (/https?:\/\//.test(normalized) || normalized.includes("www."))
        return true;
    const webSiteHint = /(youtube|google|wikipedia|github|amazon|reddit|discord|x\.com|twitter|chatgpt|gemini|qwen)/.test(normalized);
    const webActionHint = /\b(search|find|click|result|browser|website|web page|page|tab|navigate|open url|open site)\b/.test(normalized);
    return webSiteHint || webActionHint;
}
function looksLikeGenericBrowserOpenIntent(task) {
    const normalized = String(task || "").trim();
    if (!normalized)
        return false;
    if (taskLikelyTargetsDesktop(normalized))
        return false;
    if (taskLikelyRequiresWorkspaceAction(normalized) || taskLikelyReferencesWorkspaceArtifacts(normalized))
        return false;
    const folderQuery = extractOpenFolderQuery(normalized);
    if (folderQuery && looksLikeFolderIntent(folderQuery))
        return false;
    const directTarget = extractDirectOpenTarget(normalized);
    if (!directTarget)
        return false;
    if (looksLikeFolderIntent(directTarget))
        return false;
    if (/^[a-z]:\\?$/i.test(directTarget) || /^[a-z]:[\\/]/i.test(directTarget) || /[\\/]/.test(directTarget))
        return false;
    if (/\b(notepad|calculator|calc|file explorer|explorer|discord|slack|outlook|mail|terminal|cmd|powershell|chrome|brave|edge|firefox|spotify|steam|word|excel|powerpoint|paint)\b/i.test(directTarget)) {
        return false;
    }
    if (/^(?:please\s+)?(?:open|show|go\s+to|navigate\s+to|search(?:\s+for)?|find|look\s+up|watch|play|listen(?:\s+to)?)\b/i.test(normalized)) {
...
function buildBrowserMissionToolCall(task, step) {
...
function taskLikelyRequiresWorkspaceAction(task) {
    return /\b(fix|implement|edit|write|create|update|refactor|patch|modify|run tests?|test|verify|validation|lint)\b/i.test(task);
}
function taskLikelyRequiresDesktopVerification(task) {
    const normalized = String(task || "").toLowerCase();
    if (!normalized)
        return false;
    return /\b(type|write|draft|message|send|calculate|compute|divide|times|multiply|plus|minus|result|navigate|go to|select|read)\b/i.test(normalized);
}
function taskLikelyTargetsDesktop(task) {
    const normalized = String(task || "").toLowerCase();
    if (!normalized)
        return false;
...
function shouldForceInitialWorkspaceBootstrap(input) {
    if (!input.workspaceRoot)
        return false;
    if (input.hasPendingToolCall)
        return false;
    if (input.priorToolResultCount > 0)
        return false;
    if (input.completionStatus !== "complete")
        return false;
    if (input.taskSpeedClass === "chat_only")
        return false;
    if (taskRequestsValidation(input.task))
        return false;
    if (taskLikelyRequiresWorkspaceAction(input.task))
        return true;
...
function looksLikeInteractiveTerminalShortcut(task) {
    const normalized = task.trim().toLowerCase();
    if (!normalized)
        return false;
    if (/\binteractive terminal\b/.test(normalized))
        return true;
    if (/\bterminal session\b/.test(normalized))
        return true;
    if (/\bpersistent shell\b/.test(normalized))
        return true;
    if (/\binteractive shell\b/.test(normalized))
        return true;
    if (/\bpython repl\b/.test(normalized))
        return true;
    if (/\bnode repl\b/.test(normalized))
        return true;
    return /\b(start|open|launch|use)\b/.test(normalized) && /\b(?:terminal|shell|repl)\b/.test(normalized);
}
function inferInteractiveTerminalShortcutShell(task) {
    if (/\bpwsh(?:\.exe)?\b/i.test(task))
        return "pwsh";
    if (/\bpowershell(?:\.exe)?\b/i.test(task))
        return "powershell";
    if (/\bcmd(?:\.exe)?\b/i.test(task) || /\bcommand prompt\b/i.test(task))
        return "cmd";
    return undefined;
}
function cleanInteractiveTerminalShortcutInput(input) {
    const trimmed = input.trim();
    const quotedMatch = trimmed.match(/^['"`](.*)['"`]$/);
    return quotedMatch ? quotedMatch[1].trim() : trimmed;
}
function extractInteractiveTerminalShortcutPlan(task) {
    if (!looksLikeInteractiveTerminalShortcut(task))
        return null;
    const inputs = [];
...
function normalizeInteractiveTerminalShortcutOutput(output, recentInput) {
    ...
    const lines = raw
        ...
        if (!trimmed)
            return false;
        if (normalizedInput && trimmed.toLowerCase() === normalizedInput)
            return false;
        if (/^(?:PS\s+[A-Z]:\\.*>|[A-Z]:\\.*>|.*[>$#])$/i.test(trimmed) && trimmed.length <= 180)
            return false;
        return true;
    });
    return lines.join("\n").trim();
}
function isTurnBudgetTimeoutError(error) {
    const message = error instanceof Error ? error.message : String(error || "");
    return /turn budget exceeded|timed out waiting|timed out/i.test(message);
}
function isTransientNetworkTurnError(error) {
    const message = error instanceof Error ? error.message : String(error || "");
    return /transient_api_failure|fetch failed|econn|connection refused|enotfound|socket|network|service unavailable|bad gateway|gateway timeout/i.test(message);
}
function isRecoverableLocalGatewayTurnError(error) {
    const message = error instanceof Error ? error.message : String(error || "");
    if (isTurnBudgetTimeoutError(error))
        return true;
    return /provider_credits_exhausted|router_blocked|tool_schema_incompatible|transient_api_failure|unknown_provider_failure|portal_auth_invalid|usage limit has been reached|usage_limit_reached|insufficient_quota|rate.?limit|gateway unreachable|connection refused|http error 401|unauthorized|authentication token|signing in again/i.test(message);
}
function markEscalation(run, stage, reason) {
...
function buildEscalationBlockedEnvelope(input) {
    return attachHostMetadata({
        ...
        final: "Binary paused because this turn exceeded the fast execution budget and deterministic fallback routes were exhausted.",
        ...
        lastMeaningfulProof: typeof input.run.finalEnvelope?.lastMeaningfulProof === "string" && input.run.finalEnvelope.lastMeaningfulProof.trim()
...
function selectNextModelCandidate(current, candidates, attemptedAliases) {
    for (const candidate of candidates) {
        if (current && candidate.alias === current.alias)
            continue;
        if (attemptedAliases.has(candidate.alias))
            continue;
        return candidate;
    }
    return null;
}
function isFreeModelCandidate(candidate) {
...
function isOpenRouterCandidate(candidate) {
    return String(candidate?.provider || "").trim().toLowerCase() === "openrouter";
}
function isChatGptPortalCandidate(candidate) {
    return String(candidate?.provider || "").trim().toLowerCase() === "chatgpt_portal";
}
function isOpenRouterFreeCandidate(candidate) {
    if (!candidate || !isOpenRouterCandidate(candidate))
        return false;
    const model = String(candidate.model || "").trim().toLowerCase();
    return model.endsWith(":free") || model === "openrouter/free";
}
function isOpenRouterFreeCapacityError(error) {
    const message = error instanceof Error ? error.message : String(error || "");
    return /http error 429|http error 5\d\d|too many requests|rate.?limit|quota exceeded|resource_exhausted|provider_credits_exhausted|insufficient_quota|bad gateway|gateway timeout|service unavailable|temporarily unavailable|capacity exhausted|retry in \d+/i.test(message);
}
function isPortalBridgeAuthError(error) {
    const message = error instanceof Error ? error.message : String(error || "");
    return /portal_auth_invalid|http error 401|unauthorized|authentication token|could not parse your authentication token|signing in again/i.test(message);
}
function buildOpenRouterFreeFallbackCandidates(preferred, candidates) {
    const filtered = candidates.filter((candidate) => isOpenRouterFreeCandidate(candidate));
    if (!filtered.length)
        return [preferred];
    return [
        preferred,
        ...filtered.filter((candidate) => candidate.alias !== preferred.alias),
    ];
}
function shouldAttemptOpenRouterFreeFallback(input) {
    if (!isOpenRouterFreeCandidate(input.current))
        return false;
    if (!isOpenRouterFreeCapacityError(input.error))
        return false;
    return input.candidates.some((candidate) => candidate.alias !== input.current?.alias && !input.attemptedAliases.has(candidate.alias));
}
function shouldAttemptExplicitCodexCapacityFallback(input) {
    if (!input.explicitRequestedUserModel)
        return false;
    if (!isChatGptPortalCandidate(input.current))
        return false;
    if (!isOpenRouterFreeCapacityError(input.error) && !isPortalBridgeAuthError(input.error))
        return false;
    return input.candidates.some((candidate) => candidate.alias !== input.current?.alias && !input.attemptedAliases.has(candidate.alias));
}
function summarizeOpenRouterFreeExhaustion(input) {
    if (!isOpenRouterFreeCandidate(input.current))
        return null;
    if (!isOpenRouterFreeCapacityError(input.error))
        return null;
    const freeCandidates = input.candidates.filter((candidate) => isOpenRouterFreeCandidate(candidate));
    if (!freeCandidates.length)
        return null;
    const attemptedCount = freeCandidates.filter((candidate) => input.attemptedAliases.has(candidate.alias) || candidate.alias === input.current?.alias).length;
    const hasRemaining = freeCandidates.some((candidate) => !input.attemptedAliases.has(candidate.alias) && candidate.alias !== input.current?.alias);
    if (hasRemaining)
        return null;
    return attemptedCount > 1
        ? `OpenRouter free is busy right now. Binary tried ${attemptedCount} free backup models and they were all rate-limited. Please retry in a minute.`
        : "OpenRouter free is busy right now. The current free model is rate-limited. Please retry in a minute.";
}
function resolveFixedPolicyCandidate(candidates, fixedModelAlias) {
...
function resolvePolicyFirstTurnBudgetMs(policy, taskSpeedClass, task) {
    if (taskLikelyNeedsTerminalRuntime(task)) {
        return Math.max(policy.latencyBudgetsMs.deepCode, 45_000);
    }
    if (taskSpeedClass === "deep_code")
        return policy.latencyBudgetsMs.deepCode;
    if (taskLikelyTargetsDesktop(task))
...
function setTimingOnce(run, key, at) {
    if (!run.timingState)
        return;
...
function debugHostProgress(runId, message) {
    if (String(process.env.BINARY_HOST_DEBUG_PROGRESS || "").trim() !== "1")
        return;
...
function inferFocusedRoots(input) {
    ...
    if (explicitFocusWorkspace)
        reasonParts.push("focus_workspace=request.focusWorkspaceRoot");
    else if (explicitWorkspace)
        reasonParts.push("focus_workspace=request.workspaceRoot");
    else if (preferredFocusWorkspace)
        reasonParts.push("focus_workspace=preferences.focusWorkspaceRoot");
    else
        reasonParts.push("focus_workspace=none");
    if (explicitFocusRepo)
        reasonParts.push("focus_repo=request.focusRepoRoot");
    else if (preferredFocusRepo)
        reasonParts.push("focus_repo=preferences.focusRepoRoot");
    else if (focusedWorkspaceRoot)
        reasonParts.push("focus_repo=fallback_workspace_root");
    else
        reasonParts.push("focus_repo=none");
...
function isBinaryTool(name) {
    return HOST_BINARY_TOOLS.includes(name);
}
function isBinaryMutationTool(name) {
    return name === "patch_binary" || name === "write_binary_file";
}
function isBinaryInspectTool(name) {
    return isBinaryTool(name) && !isBinaryMutationTool(name);
}
function getBinaryTargetPath(argumentsValue) {
    return typeof argumentsValue.path === "string" ? argumentsValue.path.trim() : "";
}
function isBinaryRawDevicePath(targetPath) {
    const normalized = String(targetPath || "").trim();
    return /^\\\\\.\\/.test(normalized) || /^\\\\\?\\GLOBALROOT\\/i.test(normalized) || /^\/dev\//.test(normalized);
}
function isBinaryProtectedPath(targetPath) {
...
function classifyBinaryTargetRisk(targetPath) {
    const normalized = String(targetPath || "").trim();
    const extension = path.extname(normalized).toLowerCase();
    if (isBinaryRawDevicePath(normalized) ||
...
function taskRequestsValidation(task) {
    return /\b(run(?:\s+the)?\s+tests?|tests?\s+(?:until\s+they\s+pass|suite|pass)|test suite|validate|validation|lint|verify|proof)\b/i.test(task);
}
...
function normalizeMachineSearchText(value) {
    return String(value || "")
        .toLowerCase()
        .replace(/[_-]+/g, " ")
        .replace(/[^a-z0-9\s]+/g, " ")
...
function cleanOpenTargetQuery(query) {
    return String(query || "")
...
function normalizeFolderAlias(value) {
    return normalizeMachineSearchText(value);
}
const KNOWN_FOLDER_TARGETS = [
    {
...
const KNOWN_FOLDER_ALIAS_RESOLVERS = new Map();
...
const directFolderResolutionCache = new Map();
function readCachedFolderResolution(query) {
    const key = normalizeFolderAlias(query);
    if (!key)
        return null;
    return directFolderResolutionCache.get(key) ?? null;
}
function writeCachedFolderResolution(query, resolvedPath) {
    const key = normalizeFolderAlias(query);
...
function looksLikeFolderIntent(query) {
    ...
    if (!normalized)
        return false;
    if (/^[a-z]:\\?$/i.test(cleaned) || /^[a-z]:[\\/]/i.test(cleaned) || /[\\/]/.test(cleaned))
        return true;
    if (/\b[a-z]\s+drive\b/i.test(cleaned))
        return true;
    if (KNOWN_FOLDER_ALIAS_RESOLVERS.has(normalized))
        return true;
...
function extractOpenFolderQuery(task) {
    const normalized = String(task || "").trim();
    if (!normalized)
        return null;
    const folderFirst = normalized.match(/^(?:please\s+)?(?:open|show|focus|launch)\s+(.+?)\s+(?:folder|directory)(?:\s+please)?(?:\s*<3)?$/i);
    if (folderFirst?.[1])
        return cleanOpenTargetQuery(folderFirst[1]);
    const folderLast = normalized.match(/^(?:please\s+)?(?:open|show|focus|launch)\s+(?:the\s+)?(?:folder|directory)\s+(.+?)(?:\s+please)?(?:\s*<3)?$/i);
    if (folderLast?.[1])
        return cleanOpenTargetQuery(folderLast[1]);
    return null;
}
function extractDirectOpenTarget(task) {
    const normalized = String(task || "").trim();
    if (!normalized)
        return null;
...
async function listExistingDriveRoots() {
    if (process.platform !== "win32")
        return [];
...
function buildFolderNameVariants(query) {
...
async function resolveDirectFolderTarget(query) {
...
async function findExactFolderMatch(query, roots) {
...
async function searchFolderMatch(rootPath, query, maxDepth = 6, maxDirs = 3000) {
    ...
    const queue = [{ dir: normalizeWorkspacePath(rootPath), depth: 0 }];
    const seen = new Set();
    let scanned = 0;
    let best = null;
    while (queue.length && scanned < maxDirs) {
        const next = queue.shift();
        if (seen.has(next.dir))
            continue;
        seen.add(next.dir);
        scanned += 1;
        ...
        for (const entry of entries) {
            ...
            score -= next.depth * 6;
            if (/\\appdata\\/i.test(fullPath))
                score -= 40;
            if (/\\programdata\\/i.test(fullPath))
                score -= 25;
            if (/\\renpy\\/i.test(fullPath))
                score -= 20;
...
async function searchExactFolderNameMatch(rootPath, query, maxDepth = 5, maxDirs = 2500) {
    if (!(await pathExists(rootPath)))
        return null;
    const normalizedVariants = new Set(buildFolderNameVariants(query)
        .map((value) => normalizeMachineSearchText(value))
        .filter(Boolean));
    if (!normalizedVariants.size)
        return null;
    const queue = [{ dir: normalizeWorkspacePath(rootPath), depth: 0 }];
    const seen = new Set();
    let scanned = 0;
...
async function resolveLikelyFolderPath(query, run) {
...
async function tryDirectMachineShortcut(input) {
    ...
    if (taskSpeedClass !== "simple_action")
        return false;
    ...
    try {
        if (interactiveTerminalPlan && canUseInteractiveTerminal) {
            ...
            const visibleOutput = normalizeInteractiveTerminalShortcutOutput(latestOutput, latestInput) ||
                normalizeInteractiveTerminalShortcutOutput(started.output);
            await emitHostStatus(run, `Binary started interactive terminal session ${latestSession.sessionId} directly from the local machine lane.`, attachedRes, {
                shortcutKind: "interactive_terminal",
                sessionId: latestSession.sessionId,
        ...
        if (interactiveTerminalPlan) {
            await emitHostStatus(run, `Binary could not start the direct interactive terminal shortcut. Cause: ${error instanceof Error ? error.message : String(error)}`, attachedRes, {
                shortcutKind: "interactive_terminal",
...
function isWorkspaceMutationToolName(name) {
    return (name === "edit" ||
        name === "write_file" ||
        name === "mkdir" ||
        name === "patch_binary" ||
...
function taskLikelyRequiresBrowserVerification(task) {
    return /\b(verify|verification|confirm|proof|result|title|url|extract|login|form|submit|complete|open|search|find|watch|play|listen|go to|navigate)\b/i.test(String(task || ""));
}
...
function evaluateBrowserProofState(run) {
    ...
    const required = taskLikelyTargetsBrowser(run.request.task) ||
...
function getLatestDesktopMetadataValue(run, key) {
    for (let index = run.toolResults.length - 1; index >= 0; index -= 1) {
        const toolResult = run.toolResults[index];
        if (!toolResult || !String(toolResult.name || "").startsWith("desktop_"))
            continue;
        const data = toolResult.data && typeof toolResult.data === "object" ? toolResult.data : null;
        if (!data)
            continue;
...
function buildDesktopProofClosureSummary(run, proof) {
    ...
    const verificationText = !proof.requiresVerification || proof.hasVerificationProof
        ? "verified"
        : "verification not fully confirmed";
    const cleanupCount = typeof run.lastExecutionState?.cleanupClosedCount === "number"
...
async function tryCompleteDesktopRunFromProof(run, envelope, reason, attachedRes) {
    const desktopProof = evaluateDesktopProofState(run);
    const desktopActionObserved = desktopProof.hasActionProof ||
        run.toolResults.some((toolResult) => {
            if (!toolResult.ok || !String(toolResult.name || "").startsWith("desktop_"))
                return false;
            return !isObserveTool(String(toolResult.name || ""));
        });
    const desktopTargetKnown = Boolean(desktopProof.required ||
    ...
    const verificationRequired = desktopProof.requiresVerification || Boolean(run.lastExecutionState?.verificationRequired === true);
    const verificationPassed = desktopProof.hasVerificationProof || Boolean(run.lastExecutionState?.verificationPassed === true);
    ...
    await emitHostStatus(run, "Binary finished this desktop run from verified local proof.", attachedRes, {
...
async function attemptLocalCompletionProof(run, envelope, executor, attachedRes) {
    ...
    run.finalEnvelope = attachHostMetadata({
        ...
        missingRequirements: [],
        pendingToolCall: null,
        closureSummary: typeof envelope.closureSummary === "string" && envelope.closureSummary.trim()
...
function resolveWorkspaceTrustGrant(preferences, workspaceRoot) {
    const resolvedWorkspaceRoot = normalizeOptionalPath(workspaceRoot);
    if (!resolvedWorkspaceRoot)
        return null;
    const explicitGrant = preferences.trustedWorkspaces.find((grant) => normalizeWorkspacePath(grant.path) === resolvedWorkspaceRoot) || null;
    if (explicitGrant)
        return explicitGrant;
    if (preferences.machineTrustMode === "full_machine_mutate") {
        return {
            ...
            network: "allow",
            elevated: "allow",
            grantedAt: "implicit_full_machine_mutate",
        };
    }
    return null;
}
function deriveEffectiveTrustMode(preferences, workspaceRoot) {
    if (workspaceRoot)
        return deriveWorkspaceTrustMode(resolveWorkspaceTrustGrant(preferences, workspaceRoot));
    if (preferences.machineTrustMode === "full_machine_mutate")
        return "trusted_full_access";
    if (preferences.machineTrustMode === "home_mutate")
        return "trusted_prompt_commands";
    return "trusted_read_only";
}
...
function sanitizeToolResultForContinue(toolResult) {
    ...
    if (toolResult.data && typeof toolResult.data === "object") {
        ...
        if (typeof data.output === "string")
            data.output = truncateText(data.output, 12_000) || "";
        if (typeof data.content === "string")
            data.content = truncateText(data.content, 16_000) || "";
        if (typeof data.bytesBase64 === "string")
            data.bytesBase64 = truncateText(data.bytesBase64, 8_000) || "";
        if (typeof data.hexPreview === "string")
            data.hexPreview = truncateText(data.hexPreview, 2_000) || "";
        if (typeof data.asciiPreview === "string")
            data.asciiPreview = truncateText(data.asciiPreview, 2_000) || "";
        next.data = data;
    }
    return next;
}
...
function rememberHostedTransportAvailability(baseUrl, available, reason) {
...
async function probeHostedTransportAvailability(baseUrl) {
...
async function withSurfaceContextTimeout(promise, fallback, timeoutMs) {
    let timer = null;
    try {
...
async function shouldPreferLocalGatewayAssist(baseUrl, modelCandidate, runtime) {
...
function resolveRunExecutionConfig(input) {
    ...
    const explicitLane = input.run.request.executionLane || input.preferredExecutionLane;
    const shouldUseAdaptiveLaneSelection = Boolean(explicitLane) ||
...
function buildLocalGatewayToolTrace(run) {
...
function mapLocalGatewayTurnToEnvelope(turn, availableTools, step) {
    const toolCall = turn.toolCall && typeof turn.toolCall === "object" ? turn.toolCall : null;
    const adapter = typeof turn.adapter === "string" ? turn.adapter : "text_actions";
    const pendingToolCall = toolCall && typeof toolCall.name === "string"
        ? {
            step,
            adapter,
            requiresClientExecution: true,
        ...
        : null;
    const progressState = turn.progressState && typeof turn.progressState === "object"
        ? turn.progressState
        : null;
    ...
    const fallbackCount = typeof turn.fallbackCount === "number"
        ...
        : typeof turn.fallbackAttempt === "number"
            ...
            : Array.isArray(turn.fallbackTrail)
                ...
                : undefined;
    const delegationUsed = Object.prototype.hasOwnProperty.call(turn, "delegationUsed") && typeof turn.delegationUsed === "boolean"
        ? turn.delegationUsed
        : undefined;
    const delegationReason = typeof turn.delegationReason === "string" ? turn.delegationReason : undefined;
    const childCount = typeof turn.childCount === "number" && Number.isFinite(turn.childCount)
        ? Math.max(0, Math.round(turn.childCount))
        : undefined;
    const completedChildren = typeof turn.completedChildren === "number" && Number.isFinite(turn.completedChildren)
        ? Math.max(0, Math.round(turn.completedChildren))
        : undefined;
    const failedChildren = typeof turn.failedChildren === "number" && Number.isFinite(turn.failedChildren)
        ? Math.max(0, Math.round(turn.failedChildren))
        : undefined;
    const childSummaries = normalizeDelegationChildSummaries(turn.childSummaries);
    const adapterMode = turn.adapterMode === "force_binary_tool_adapter" || turn.adapterMode === "auto"
        ? turn.adapterMode
        : undefined;
    const latencyPolicy = turn.latencyPolicy === "detached_15s_cap" || turn.latencyPolicy === "default"
        ? turn.latencyPolicy
        : undefined;
    const smallModelForced = Object.prototype.hasOwnProperty.call(turn, "smallModelForced")
        ? turn.smallModelForced === true
        : undefined;
    const modelRoutingMode = turn.modelRoutingMode === "single_fixed_free" ? "single_fixed_free" : undefined;
    ...
    return {
        ...
        orchestratorVersion: typeof turn.orchestratorVersion === "string" || turn.orchestratorVersion === null
            ? turn.orchestratorVersion
            : undefined,
        executionLane: turn.executionLane === "local_interactive" ||
            turn.executionLane === "openhands_headless" ||
            turn.executionLane === "openhands_remote"
            ? turn.executionLane
            : undefined,
        runtimeTarget: turn.runtimeTarget === "local_native" || turn.runtimeTarget === "sandbox" || turn.runtimeTarget === "remote"
            ? turn.runtimeTarget
            : undefined,
        toolBackend: turn.toolBackend === "openhands_native" || turn.toolBackend === "binary_host"
            ? turn.toolBackend
            : undefined,
        pluginPacks: Array.isArray(turn.pluginPacks) ? turn.pluginPacks : undefined,
        skillSources: Array.isArray(turn.skillSources) ? turn.skillSources : undefined,
        ...
        approvalState: turn.approvalState === "autonomous" ||
            ...
            ? turn.approvalState
            : undefined,
        worldContextUsed: turn.worldContextUsed && typeof turn.worldContextUsed === "object"
            ? turn.worldContextUsed
            : undefined,
        final: typeof turn.final === "string" ? turn.final : "",
        pendingToolCall,
        completionStatus: pendingToolCall ? "incomplete" : "complete",
        ...(typeof delegationUsed === "boolean" ? { delegationUsed } : {}),
        ...(typeof delegationReason === "string" ? { delegationReason } : {}),
        ...(typeof childCount === "number" ? { childCount } : {}),
        ...(typeof completedChildren === "number" ? { completedChildren } : {}),
        ...(typeof failedChildren === "number" ? { failedChildren } : {}),
        ...
        receipt: turn.modelCandidate && typeof turn.modelCandidate === "object"
            ? {
...
async function consumeLocalGatewaySseTurn(response, onEvent, allowPartialTerminalFallback = false) {
    if (!response.body) {
    ...
    try {
        while (true) {
            ...
            while (true) {
                ...
                if (onEvent && eventName && eventName !== "gateway.result" && eventName !== "gateway.error") {
                ...
                if (eventName === "gateway.error" && payloadData && typeof payloadData === "object" && !Array.isArray(payloadData)) {
                    await flushPartial(true);
                    const typed = payloadData;
                    const detail = (typeof typed.details === "string" && typed.details.trim()) ||
...
async function runLocalGatewayAssist(input) {
    ...
    const detachedTurnBudgetMs = input.latencyPolicy === "detached_15s_cap"
        ...
        : input.latestToolResult
            ? 90_000
            : 45_000;
    const fastFailPolicy = initialTurn &&
        input.adapterMode === "force_binary_tool_adapter" &&
        input.latencyPolicy === "detached_15s_cap";
    const effectiveRoutePolicy = input.run.request.detach === true ||
    ...
    const body = {
        ...
        delegation: input.delegation && input.delegation.enabled === false
            ? { ...DEFAULT_HOSTED_DELEGATION_CONFIG, enabled: false }
            : input.delegation || DEFAULT_HOSTED_DELEGATION_CONFIG,
    ...
    try {
        if (isSseContentType(response)) {
            parsed = await consumeLocalGatewaySseTurn(response, async (event) => {
                if (input.onEvent) {
                    await input.onEvent(event);
                }
                const patch = buildDelegationEnvelopePatchFromEvent(event);
                if (patch) {
                    applyEnvelopeToRun(input.run, patch);
                    await persistHostRun(input.run);
                    await agentJobManager.syncFromRun(input.run);
                }
...
function isWaitingForUserInput(run) {
    return Boolean(run && run.status === "paused" && run.pendingUserInputRequest);
}
function extractDirectChatText(payload) {
...
function supportsDirectFastChat(candidate) {
    if (!candidate?.baseUrl || !candidate.apiKey)
        return false;
    const provider = getProviderCatalogEntry(candidate.provider);
    if (!provider)
        return false;
...
function resolveTrustedCorsOrigin(req) {
...
function setResponseCorsOrigin(res, origin) {
    res[RESPONSE_CORS_ORIGIN] = origin;
}
function withCors(res) {
    const allowedOrigin = res[RESPONSE_CORS_ORIGIN];
...
async function scheduleOpenHandsWarmup() {
    if (openHandsWarmupScheduled)
        return;
...
async function executeHostedAgentProbeTurn(input) {
    ...
    const localGatewayModelCandidate = resolvedProbeModel.source === "user_connected"
    ...
    if (localGatewayModelCandidate) {
        ...
        const body = {
            ...
            context: worldContext || repoContext || verificationPlan
                ? {
        ...
        if (!response.ok) {
            throw new Error((typeof parsed.details === "string" && parsed.details.trim()) ||
                (typeof parsed.error === "string" && parsed.error.trim()) ||
        ...
        return {
            ...
            modelCandidate: parsed.modelCandidate && typeof parsed.modelCandidate === "object"
                ? parsed.modelCandidate
                : null,
            ...
            fallbackTrail: Array.isArray(parsed.fallbackTrail) ? parsed.fallbackTrail : [],
        };
    }
...
function defaultPreferences() {
    return {
        ...
        preferredExecutionLane: undefined,
        defaultPluginPacks: [],
...
async function loadPreferences() {
    ...
    return {
        ...
        preferredExecutionLane: existing?.preferredExecutionLane === "local_interactive" ||
            existing?.preferredExecutionLane === "openhands_headless" ||
            existing?.preferredExecutionLane === "openhands_remote"
            ? existing.preferredExecutionLane
            : defaultValue.preferredExecutionLane,
        ...
        machineRootPath: typeof existing?.machineRootPath === "string" && existing.machineRootPath.trim()
            ? normalizeWorkspacePath(existing.machineRootPath)
            : defaultValue.machineRootPath,
        machineRootMode: existing?.machineRootMode === "home_root" || existing?.machineRootMode === "hybrid_root"
            ? existing.machineRootMode
            : defaultValue.machineRootMode,
        machineTrustMode: existing?.machineTrustMode === "observe_first" ||
        ...
        systemPathScope: existing?.systemPathScope === "excluded" ||
            existing?.systemPathScope === "included" ||
            existing?.systemPathScope === "prompt"
            ? existing.systemPathScope
            : defaultValue.systemPathScope,
        focusWorkspaceRoot: typeof existing?.focusWorkspaceRoot === "string" && existing.focusWorkspaceRoot.trim()
            ? normalizeWorkspacePath(existing.focusWorkspaceRoot)
            : undefined,
        focusRepoRoot: typeof existing?.focusRepoRoot === "string" && existing.focusRepoRoot.trim()
...
async function setStoredSecretFileValue(key, value) {
...
async function getConnectionSecretMapFromFile() {
    const current = await getStoredSecretFile();
...
async function setConnectionSecretMapFromFile(map) {
...
async function getConnectionSecretRecord(connectionId) {
...
async function setConnectionSecretRecord(connectionId, secret) {
    const id = String(connectionId || "").trim();
    const normalized = {
...
async function clearConnectionSecretRecord(connectionId) {
    const id = String(connectionId || "").trim();
    const keytar = await loadOptionalKeytar();
    if (keytar) {
...
async function getConnectionSecrets(records) {
    const entries = await Promise.all(records.map(async (record) => [record.id, await getConnectionSecretRecord(record.id)]));
    return Object.fromEntries(entries);
}
async function getFallbackProviderApiKey(providerId, records) {
    const normalizedProviderId = String(providerId || "").trim();
    if (!normalizedProviderId)
        return null;
    const catalog = getProviderCatalogEntry(normalizedProviderId);
    if (!catalog || catalog.authStrategy !== "api_key")
        return null;
    if (records.some((record) => record.providerId === normalizedProviderId))
        return null;
    const fileMap = await getConnectionSecretMapFromFile();
...
async function testRemoteConnection(record, secret) {
...
async function listConnectionViews(preferences) {
...
function sanitizeProviderValidationMessage(providerId, status, raw) {
...
function buildProviderValidationHeaders(providerId, apiKey) {
...
function findProviderRecord(preferences, providerId) {
...
async function upsertProviderConnection(input) {
    const catalog = getProviderCatalogEntry(input.providerId);
    if (!catalog)
        return { ok: false, statusCode: 404, message: "Unknown provider." };
    ...
    const apiKey = String(input.apiKey || "").trim();
    if (!apiKey)
        return { ok: false, statusCode: 400, message: `${catalog.apiKeyLabel} is required.` };
    let preferences = await loadPreferences();
...
async function persistOAuthProviderConnection(input) {
    ...
    const runtimeReadinessReason = !validation.ok && allowLinkedButLimited
...
async function persistBrowserSessionProviderConnection(input) {
    ...
    const defaultModel = runtime.runtimeReady &&
...
async function startProviderOAuthSession(input) {
    ...
    const metadata = {
        ...
        projectHint: parseProviderProjectHint({
            providerId: catalog.id,
            baseUrl,
            fallbackProjectId: catalog.id === "gemini"
    ...
    const session = catalog.authStrategy === "oauth_device"
...
async function finalizeCompletedOAuthSession(session) {
    if (!session.tokenSet) {
...
async function ensureOAuthProviderConnectionFresh(preferences) {
    let mutated = false;
    for (const record of preferences.connections) {
        if (record.authMode !== "oauth" || !record.providerId)
            continue;
        const catalog = getProviderCatalogEntry(record.providerId);
        if (!catalog || !isOAuthProviderCatalog(catalog))
            continue;
        const secret = await getConnectionSecretRecord(record.id);
...
async function testProviderConnection(providerId) {
    ...
    if (isBrowserSessionProviderCatalog(catalog)) {
        ...
        const provider = (await listProviderViews({ preferences })).find((item) => item.id === providerId);
        if (!provider)
            return { ok: false, statusCode: 500, message: "Provider was tested but could not be loaded." };
        return {
            ok: true,
            provider,
            test: {
    ...
    const secret = await getConnectionSecretRecord(record.id);
    const validation = record.authMode === "oauth"
...
async function setDefaultProvider(providerId) {
    ...
    const existing = findProviderRecord(preferences, providerId);
    if (!existing)
        return { ok: false, statusCode: 404, message: "Connect the provider before making it default." };
    preferences.defaultProviderId = providerId;
    preferences.connections = preferences.connections.map((record) => record.providerId ? { ...record, preferredForModels: record.providerId === providerId } : record);
    await savePreferences(preferences);
    return {
        ok: true,
...
function getPublicHostBaseUrl(req) {
...
function isOAuthProviderCatalog(catalog) {
...
function isBrowserSessionProviderCatalog(catalog) {
    return Boolean(catalog &&
...
function buildOauthAvailabilityMessage(catalog) {
    if (!isOAuthProviderCatalog(catalog)) {
...
function buildLinkedAccountLabel(secret) {
    const idTokenClaims = decodeJwtPayload(secret.idToken);
    const account = String(secret.accountHint ||
        idTokenClaims?.email ||
...
function buildProviderRouteMetadata(catalog, baseUrl) {
...
function resolveOAuthRuntimeBaseUrl(catalog, baseUrl) {
...
function buildBrowserSessionRuntimeFallbackReason(catalog) {
    if (catalog.id === "chatgpt_portal") {
...
function collectBrowserSessionRuntimeBridgeCandidates(catalog, baseUrl) {
...
async function resolveBrowserSessionRuntimeResolution(input) {
    ...
    for (const bridgeBaseUrl of collectBrowserSessionRuntimeBridgeCandidates(input.catalog, input.baseUrl)) {
        ...
        return {
            ...
            routeLabel: input.catalog.id === "chatgpt_portal"
...
async function fetchProviderOauthModels(input) {
    ...
    const tryUrls = input.provider.id === "gemini"
        ? [
    ...
    for (const target of tryUrls) {
        try {
            ...
            if (modelIds.length)
                return Array.from(new Set(modelIds)).slice(0, 120);
        }
        catch {
...
function resolveRequestedProviderModelAlias(requestedModel, candidates) {
    ...
    const matched = candidates.find((candidate) => candidate.alias.toLowerCase() === lowered ||
...
async function materializeEnabledConnections(preferences, mode) {
    if (mode === "debug")
        return undefined;
    const secrets = await getConnectionSecrets(preferences.connections);
    const eligible = preferences.connections.filter((record) => {
        if (isProviderConnection(record))
            return false;
        if (!record.enabled)
            return false;
        if (getConnectionStatus(record, secrets[record.id]) !== "connected")
            return false;
...
async function createHostToolExecutor(input) {
    ...
    const desktopExecutor = new DesktopToolExecutor(machineAutonomyController, input.preferences.machineAutonomy, executionController, nativeAppRuntime, input.task, {
    ...
    const workspaceExecutor = new moduleRef.CliToolExecutor(input.workspaceRoot || input.run.machineRootPath || process.cwd(), input.workspaceRoot ? extractRequestedProjectRoot(input.task) : null);
    const terminalExecutor = {
        execute: async (pendingToolCall) => {
            const toolName = String(pendingToolCall.toolCall.name || "");
        ...
        return {
            async execute(pendingToolCall) {
                ...
                if (String(pendingToolCall.toolCall.name || "").startsWith("repo_")) {
                    return decorateResult(pendingToolCall, {
                        toolCallId: pendingToolCall.toolCall.id,
                        name: pendingToolCall.toolCall.name,
                        ok: false,
                        blocked: true,
...
function isWorkspaceTrusted(preferences, workspaceRoot) {
    return resolveWorkspaceTrustGrant(preferences, workspaceRoot);
}
...
function extractFallbackCount(envelope) {
...
function computeLatencyMetrics(run, envelope) {
...
function refreshRunLatencyMetrics(run, envelope) {
    if (!run.timingState)
        return;
    const metrics = computeLatencyMetrics(run, envelope);
    if (typeof metrics.queueDelayMs === "number")
        run.timingState.queueDelayMs = metrics.queueDelayMs;
    if (typeof metrics.ttfrMs === "number")
        run.timingState.ttfrMs = metrics.ttfrMs;
    if (typeof metrics.firstToolMs === "number")
        run.timingState.firstToolMs = metrics.firstToolMs;
    if (typeof metrics.totalRunMs === "number")
        run.timingState.totalRunMs = metrics.totalRunMs;
    if (typeof metrics.fallbackCount === "number")
        run.timingState.fallbackCount = metrics.fallbackCount;
}
function classifyLatencyLane(run) {
    ...
    if (hasDesktopToolTrace || taskLikelyTargetsDesktop(task))
        return "desktop";
    const speedClass = run.timingState?.taskSpeedClass;
    if (speedClass === "chat_only" || speedClass === "simple_action")
        return "chat";
    return "coding";
}
function hasBudgetTimeoutSignal(run) {
    const candidates = [
        run.takeoverReason,
...
function buildLatencyScorecard(runs) {
    ...
    const totalRun = withTiming
        .map((run) => asNonNegativeInteger(run.timingState?.totalRunMs) ??
    ...
    for (const run of withTiming) {
        ...
        const laneTotalRun = asNonNegativeInteger(run.timingState?.totalRunMs) ??
            diffMs(run.timingState?.startedAt, run.timingState?.finalAt);
        if (typeof laneTtfr === "number")
            laneBuckets[lane].ttfr.push(laneTtfr);
        if (typeof laneFirstTool === "number")
            laneBuckets[lane].firstTool.push(laneFirstTool);
        if (typeof laneTotalRun === "number")
            laneBuckets[lane].totalRun.push(laneTotalRun);
        if (hasBudgetTimeoutSignal(run)) {
            ...
            if (typeof firstTurnBudget === "number" &&
...
function attachHostMetadata(envelope, run) {
    ...
    const defaultToolBackend = inferredShortcutEnvelope || envelope.adapter === "desktop_native" ? "binary_host" : "openhands_native";
    ...
    return {
        ...
        jsonlPath: (typeof envelope.jsonlPath === "string" ? envelope.jsonlPath : undefined) ||
            ...
            null,
        ...(typeof envelope.delegationUsed === "boolean" || typeof run.delegationUsed === "boolean"
            ? { delegationUsed: envelope.delegationUsed ?? run.delegationUsed }
            : {}),
        ...(typeof envelope.delegationReason === "string" || typeof run.delegationReason === "string"
            ? { delegationReason: envelope.delegationReason || run.delegationReason }
            : {}),
        ...(typeof envelope.childCount === "number" || typeof run.childCount === "number"
            ? { childCount: envelope.childCount ?? run.childCount }
            : {}),
        ...(typeof envelope.completedChildren === "number" || typeof run.completedChildren === "number"
            ? { completedChildren: envelope.completedChildren ?? run.completedChildren }
            : {}),
        ...(typeof envelope.failedChildren === "number" || typeof run.failedChildren === "number"
            ? { failedChildren: envelope.failedChildren ?? run.failedChildren }
            : {}),
        ...(Array.isArray(envelope.childSummaries) || Array.isArray(run.childSummaries)
            ? { childSummaries: envelope.childSummaries || run.childSummaries || [] }
            : {}),
        ...(Object.prototype.hasOwnProperty.call(envelope, "userInputRequest") || run.pendingUserInputRequest
            ? { userInputRequest: envelope.userInputRequest ?? run.pendingUserInputRequest ?? null }
            : {}),
        ...
        worldContextUsed: envelope.worldContextUsed ||
...
function applyEnvelopeToRun(run, envelope) {
    ...
    if (Array.isArray(envelope.skillSources))
        run.skillSources = envelope.skillSources;
    if (typeof envelope.conversationId === "string")
        run.conversationId = envelope.conversationId;
    if (typeof envelope.persistenceDir === "string")
        run.persistenceDir = envelope.persistenceDir;
    const hasExplicitDelegationChildCount = typeof envelope.childCount === "number" && Number.isFinite(envelope.childCount);
    const hasExplicitCompletedChildren = typeof envelope.completedChildren === "number" && Number.isFinite(envelope.completedChildren);
    const hasExplicitFailedChildren = typeof envelope.failedChildren === "number" && Number.isFinite(envelope.failedChildren);
    if (typeof envelope.delegationUsed === "boolean")
        run.delegationUsed = envelope.delegationUsed;
    if (typeof envelope.delegationReason === "string")
        run.delegationReason = envelope.delegationReason;
    if (typeof envelope.childCount === "number")
        run.childCount = Math.max(0, Math.round(envelope.childCount));
    if (typeof envelope.completedChildren === "number") {
        run.completedChildren = Math.max(0, Math.round(envelope.completedChildren));
    }
    if (typeof envelope.failedChildren === "number")
        run.failedChildren = Math.max(0, Math.round(envelope.failedChildren));
    if (Object.prototype.hasOwnProperty.call(envelope, "userInputRequest")) {
        run.pendingUserInputRequest = envelope.userInputRequest || null;
    }
    if (Array.isArray(envelope.childSummaries)) {
        run.childSummaries = mergeDelegationChildSummaries(run.childSummaries, envelope.childSummaries);
    }
    if (run.childSummaries?.length) {
        if (!hasExplicitDelegationChildCount)
            run.childCount = run.childSummaries.length;
        if (!hasExplicitCompletedChildren) {
            run.completedChildren = countDelegationChildren(run.childSummaries, ["completed"]);
        }
        if (!hasExplicitFailedChildren) {
            run.failedChildren = countDelegationChildren(run.childSummaries, ["failed", "cancelled"]);
        }
    }
    run.lastExecutionState = {
        ...(run.lastExecutionState || {}),
        ...
        ...(typeof envelope.delegationUsed === "boolean" ? { delegationUsed: envelope.delegationUsed } : {}),
        ...(typeof envelope.delegationReason === "string" ? { delegationReason: envelope.delegationReason } : {}),
        ...(typeof envelope.childCount === "number" ? { childCount: Math.max(0, Math.round(envelope.childCount)) } : {}),
        ...(typeof envelope.completedChildren === "number"
            ? { completedChildren: Math.max(0, Math.round(envelope.completedChildren)) }
            : {}),
        ...(typeof envelope.failedChildren === "number"
            ? { failedChildren: Math.max(0, Math.round(envelope.failedChildren)) }
            : {}),
        ...(Array.isArray(envelope.childSummaries)
            ? {
                childSummaries: mergeDelegationChildSummaries(run.lastExecutionState?.childSummaries, envelope.childSummaries),
            }
            : {}),
    };
    if (Array.isArray(run.lastExecutionState?.childSummaries) && run.lastExecutionState.childSummaries.length) {
        if (!hasExplicitDelegationChildCount)
            run.lastExecutionState.childCount = run.lastExecutionState.childSummaries.length;
        if (!hasExplicitCompletedChildren) {
            run.lastExecutionState.completedChildren = countDelegationChildren(run.lastExecutionState.childSummaries, ["completed"]);
        }
        if (!hasExplicitFailedChildren) {
            run.lastExecutionState.failedChildren = countDelegationChildren(run.lastExecutionState.childSummaries, ["failed", "cancelled"]);
        }
    }
    const receipt = envelope.receipt && typeof envelope.receipt === "object" ? envelope.receipt : null;
    const fallbackTrail = receipt && Array.isArray(receipt.fallbackTrail)
        ? receipt.fallbackTrail
        : [];
    if (run.timingState && fallbackTrail.length > 1) {
        run.timingState.escalatedRoute = true;
    }
    if (run.timingState) {
        if (typeof envelope.plannerLatencyMs === "number")
            run.timingState.plannerLatencyMs = Math.round(envelope.plannerLatencyMs);
        if (typeof envelope.providerLatencyMs === "number")
            run.timingState.providerLatencyMs = Math.round(envelope.providerLatencyMs);
        if (typeof envelope.actionLatencyMs === "number")
            run.timingState.actionLatencyMs = Math.round(envelope.actionLatencyMs);
    }
...
function enrichPendingToolCallForUi(run, preferences, pendingToolCall) {
    ...
    const binaryApprovalRequired = pendingToolCall.toolCall.name === "patch_binary" || pendingToolCall.toolCall.name === "write_binary_file"
...
async function appendRunEvent(run, event, attachedRes) {
    ...
    if (eventName === "tool_request") {
        ...
        setTimingOnce(run, "firstVisibleTextAt", capturedAt);
        if (eventName === "final")
            setTimingOnce(run, "finalAt", capturedAt);
    }
    else if (eventName === "host.status") {
        const message = event.data && typeof event.data === "object" && typeof event.data.message === "string"
...
function sanitizeToolResultForUi(toolResult) {
    ...
    if (data && "bytesBase64" in data)
        delete data.bytesBase64;
    return {
...
function normalizeUserInputAnswer(value) {
...
function normalizeUserInputAnswers(value) {
    ...
    for (const [key, rawValue] of Object.entries(value)) {
        ...
        if (!id)
            continue;
        const answer = normalizeUserInputAnswer(rawValue);
...
function summarizeUserInputAnswers(request, answers) {
    const lines = request.questions
        .map((question) => {
        const answer = answers[question.id];
        if (!answer?.answers.length)
            return null;
        return `- ${question.question}: ${answer.answers.join("; ")}`;
    })
...
function appendPlanUserInputContext(task, request, answers) {
    const summary = summarizeUserInputAnswers(request, answers).trim();
    if (!summary)
        return task;
    return `${String(task || "").trim()}\n\nAdditional planning context from the user:\n${summary}`;
}
...
async function enforceToolPolicy(run, preferences, pendingToolCall) {
    if (isBinaryMutationTool(pendingToolCall.toolCall.name)) {
        ...
        if (isBinaryRawDevicePath(targetPath)) {
            return blockedToolResult(pendingToolCall, "Binary Host blocked a raw device mutation. Whole-machine binary writes are limited to regular files in v1.");
        }
        if (riskClass === "critical") {
            return blockedToolResult(pendingToolCall, "Binary Host blocked a critical binary mutation in a protected or boot-like location.");
        }
        if (riskClass === "high") {
            return blockedToolResult(pendingToolCall, "Binary Host requires explicit approval and dry-run proof before mutating executable or signed-style binary targets.");
        }
    }
...
function normalizeDelegationConfig(value) {
    if (!value || typeof value !== "object")
        return undefined;
    const record = value;
    const enabled = typeof record.enabled === "boolean" ? record.enabled : undefined;
    const mode = record.mode === "auto" ? "auto" : undefined;
    const maxChildren = typeof record.maxChildren === "number" && Number.isFinite(record.maxChildren)
        ? Math.max(1, Math.round(record.maxChildren))
        : undefined;
    const visibility = record.visibility === "summary_only" ? "summary_only" : undefined;
    const supportedAgentTypes = Array.isArray(record.supportedAgentTypes)
        ? record.supportedAgentTypes.filter((item) => item === "default")
        : undefined;
    if (typeof enabled !== "boolean" &&
        !mode &&
        typeof maxChildren !== "number" &&
        !visibility &&
        !supportedAgentTypes?.length) {
        return undefined;
    }
    return {
        enabled: enabled ?? DEFAULT_HOSTED_DELEGATION_CONFIG.enabled,
        mode: mode ?? DEFAULT_HOSTED_DELEGATION_CONFIG.mode,
        maxChildren: maxChildren ?? DEFAULT_HOSTED_DELEGATION_CONFIG.maxChildren,
        visibility: visibility ?? DEFAULT_HOSTED_DELEGATION_CONFIG.visibility,
        supportedAgentTypes: supportedAgentTypes?.length ? supportedAgentTypes : DEFAULT_HOSTED_DELEGATION_CONFIG.supportedAgentTypes,
    };
}
...
function buildDelegationEnvelopePatchFromEvent(event) {
    const eventName = typeof event.event === "string" ? event.event : "";
    ...
    return {
        ...
        ...(typeof data.delegationReason === "string" && data.delegationReason.trim()
            ...
            : {}),
        ...(typeof data.childCount === "number" && Number.isFinite(data.childCount)
            ? { childCount: Math.max(0, Math.round(data.childCount)) }
            : {}),
        ...(typeof data.completedChildren === "number" && Number.isFinite(data.completedChildren)
            ? { completedChildren: Math.max(0, Math.round(data.completedChildren)) }
            : {}),
        ...(typeof data.failedChildren === "number" && Number.isFinite(data.failedChildren)
            ? { failedChildren: Math.max(0, Math.round(data.failedChildren)) }
            : {}),
        ...(normalizeDelegationChildSummaries(data.childSummaries) ||
            ...
            ? {
                childSummaries: normalizeDelegationChildSummaries(data.childSummaries) ||
                    normalizeDelegationChildSummaries(data.children) ||
                    (directChildSummary ? [directChildSummary] : undefined),
            }
            : {}),
...
async function syncWorldRouteDecision(run, worldContext, taskSpeedClass, contextTier, attachedRes) {
    const topRoute = worldContext && Array.isArray(worldContext.routeRecommendations) && worldContext.routeRecommendations[0] && typeof worldContext.routeRecommendations[0] === "object"
        ? worldContext.routeRecommendations[0]
        : null;
    ...
    if (!topRoute || !kind)
        return;
    const decisionFeatures = topRoute.decisionFeatures && typeof topRoute.decisionFeatures === "object"
        ? topRoute.decisionFeatures
        : {};
    if (run.worldRouteKind === kind && run.worldContextTier === contextTier && run.worldRouteDecisionId) {
        run.lastExecutionState = {
            ...
            decisionFeatures,
            historicalSuccessWeight: typeof decisionFeatures.historicalSuccessWeight === "number" ? decisionFeatures.historicalSuccessWeight : undefined,
    ...
    await appendRunEvent(run, {
        ...
        data: {
            ...
            historicalSuccessWeight: typeof decisionFeatures.historicalSuccessWeight === "number" ? decisionFeatures.historicalSuccessWeight : null,
...
function mapToolResultToRouteOutcome(pendingToolCall, toolResult) {
...
async function emitTakeoverRequired(run, reason, attachedRes) {
    await appendRunEvent(run, {
        ...
        data: {
            ...
            reason,
            resumeToken: run.resumeToken,
            unfinishedChecklistItems: Array.isArray(run.finalEnvelope?.unfinishedChecklistItems) ? run.finalEnvelope.unfinishedChecklistItems : [],
            lastMeaningfulProof: typeof run.finalEnvelope?.lastMeaningfulProof === "string" ? run.finalEnvelope.lastMeaningfulProof : undefined,
            nextDeterministicAction: typeof run.finalEnvelope?.progressState?.nextDeterministicAction === "string"
                ? run.finalEnvelope.progressState.nextDeterministicAction
                : undefined,
            closurePhase: typeof run.finalEnvelope?.loopState?.closurePhase === "string" ? run.finalEnvelope.loopState.closurePhase : undefined,
            closureSummary: typeof run.finalEnvelope?.closureSummary === "string" ? run.finalEnvelope.closureSummary : undefined,
        },
    }, attachedRes);
}
...
async function finalizeRun(run, status, attachedRes, extra) {
    ...
    if (status === "completed" && run.finalEnvelope?.loopState?.closurePhase === "complete") {
        await appendRunEvent(run, {
            ...
            data: {
                runId: run.id,
                message: typeof run.finalEnvelope?.closureSummary === "string" && run.finalEnvelope.closureSummary.trim()
                    ? run.finalEnvelope.closureSummary
                    : "Binary completed closure for the current run.",
            },
        }, attachedRes);
    }
    if ((status === "takeover_required" || status === "failed") &&
    ...
    if (extra?.message || extra?.error) {
        ...
        const statusMessage = msg && err && !msg.includes(err) ? `${msg} Cause: ${err}` : msg || err || "Binary Host ended the run.";
...
async function executeHostRun(runId, attachedRes) {
    ...
    let localGatewayModelCandidate = null;
    let localGatewayModelCandidates = [];
    const attemptedFallbackAliases = new Set();
    try {
        ...
        const executionConfig = resolveRunExecutionConfig({
            ...
            taskSpeedClass,
            remoteConfigured: Boolean(remoteHealth?.available && remoteHealth.compatibility === "gateway_compatible"),
            preferredExecutionLane: preferences.preferredExecutionLane,
        ...
        run.request.delegation =
            run.request.delegation && run.request.delegation.enabled === false
                ? { ...DEFAULT_HOSTED_DELEGATION_CONFIG, enabled: false }
                : run.request.delegation || DEFAULT_HOSTED_DELEGATION_CONFIG;
        if (!run.timingState) {
            run.timingState = {
        ...
        run.lastExecutionState = {
            ...(run.lastExecutionState || {}),
            lane: executionConfig.executionLane === "local_interactive"
                ? "interactive-fast"
        ...
        if (await tryDirectMachineShortcut({
            run,
            preferences,
            taskSpeedClass,
            speedProfile,
        ...
        const skipDesktopSurfaceContext = deferInitialContextHydration ||
            !shouldCollectMachineSurfaceContext ||
        ...
        if (deferInitialContextHydration) {
            ...
            [desktopContext, browserContext] = await Promise.all([
                skipDesktopSurfaceContext
                    ...
                    : withSurfaceContextTimeout(collectDesktopContext({
                        machineAutonomyController,
                        policy: preferences.machineAutonomy,
                    }).catch(() => ({ platform: process.platform })), { platform: process.platform }, surfaceContextTimeoutMs),
                shouldCollectMachineSurfaceContext
                    ? withSurfaceContextTimeout(collectBrowserContext({
                        runtime: browserRuntimeController,
                        policy: preferences.machineAutonomy,
                        fast: fastSurfaceSummary,
                    }).catch(() => ({ mode: "unavailable" })), { mode: "unavailable" }, surfaceContextTimeoutMs)
        ...
        const gatewayHintsForCandidate = (candidate, initialTurn) => buildGatewayExecutionHints({
            run,
            candidate,
            orchestrationPolicy,
            taskSpeedClass,
        ...
        const fixedPolicyModelAlias = typeof orchestrationPolicy.fixedModelAlias === "string" && orchestrationPolicy.fixedModelAlias.trim()
            ? orchestrationPolicy.fixedModelAlias.trim()
            : undefined;
        const explicitRequestedUserModel = resolvedModelRequest.source === "user_connected" &&
            typeof run.request.model === "string" &&
            run.request.model.trim() &&
            run.request.model.trim().toLowerCase() !== "binary ide"
            ? orderedUserConnectedModels.find((candidate) => candidate.alias === resolvedModelRequest.model) || null
            : null;
        localGatewayModelCandidate =
            explicitRequestedUserModel ||
        ...
        localGatewayModelCandidates = localGatewayModelCandidate
            ? (() => {
                ...
                const fixedPolicyCandidate = resolveFixedPolicyCandidate(orderedUserConnectedModels, fixedPolicyModelAlias);
                const openRouterFreeFallbackCandidates = isOpenRouterFreeCandidate(localGatewayModelCandidate)
                    ? buildOpenRouterFreeFallbackCandidates(localGatewayModelCandidate, prioritized)
                    : [];
                const explicitPortalFallbackCandidates = explicitRequestedUserModel &&
                    isChatGptPortalCandidate(localGatewayModelCandidate) &&
                    fixedPolicyCandidate &&
                    isOpenRouterFreeCandidate(fixedPolicyCandidate)
                    ? [
                        localGatewayModelCandidate,
                        ...buildOpenRouterFreeFallbackCandidates(fixedPolicyCandidate, prioritized).filter((candidate) => candidate.alias !== localGatewayModelCandidate?.alias),
                    ]
                    : [];
                if (orchestrationPolicy.modelRoutingMode === "single_fixed_free" ||
                    orchestrationPolicy.fallbackEnabled !== true) {
                    if (explicitPortalFallbackCandidates.length > 1) {
                        return explicitPortalFallbackCandidates;
                    }
                    return openRouterFreeFallbackCandidates.length
                        ? openRouterFreeFallbackCandidates
                        : prioritized.slice(0, 1);
                }
                return prioritized;
            })()
            : orderedUserConnectedModels;
        const forcedSmallModelAliases = new Set(orderedUserConnectedModels
        ...
        attemptedFallbackAliases.clear();
        if (localGatewayModelCandidate?.alias) {
            attemptedFallbackAliases.add(localGatewayModelCandidate.alias);
        }
        ...
        debugHostProgress(run.id, `routing decided: source=${resolvedModelRequest.source} openhandsFirst=true candidate=${localGatewayModelCandidate?.alias || "none"}`);
        let assistTransport = "local_gateway";
        const shouldUseManagedRuntime = assistTransport === "local_gateway";
        const openhandsRuntime = shouldUseManagedRuntime
            ? await (async () => {
        ...
        if (assistTransport === "local_gateway" && !localGatewayModelCandidate) {
            const runtimeFallbackApiKey = await getFallbackProviderApiKey(typeof openhandsRuntime.currentModelCandidate?.provider === "string"
                ...
                : undefined, preferences.connections);
        ...
        if (!localGatewayModelCandidate) {
            throw new Error("Binary could not find a connected local model for the OpenHands gateway. Connect a local provider/model and try again.");
        }
        const runLocalGatewayAssistWithHints = async (args) => {
            const hints = gatewayHintsForCandidate(args.modelCandidate, !args.latestToolResult && !args.gatewayRunId);
            return await runLocalGatewayAssist({
                ...args,
                delegation: args.run.request.delegation,
        ...
        if (assistTransport === "local_gateway") {
            ...
            const forceNoTimeoutRetryChain = run.request.detach === true &&
                isSmallModelForcedCandidate(selectedCandidate, orchestrationPolicy);
            const allowSingleTimeoutFallbackForForced = allowModelFallback && forceNoTimeoutRetryChain && taskSpeedClass === "deep_code";
            ...
            while (selectedCandidate) {
                try {
                    ...
                    const openRouterFreeFallbackAllowed = shouldAttemptOpenRouterFreeFallback({
                        current: selectedCandidate,
                        candidates: localGatewayModelCandidates,
                        error,
                        attemptedAliases: attemptedFallbackAliases,
                    });
                    const explicitCodexCapacityFallbackAllowed = shouldAttemptExplicitCodexCapacityFallback({
                        current: selectedCandidate,
                        candidates: localGatewayModelCandidates,
                        error,
                        attemptedAliases: attemptedFallbackAliases,
                        explicitRequestedUserModel: Boolean(explicitRequestedUserModel),
                    });
                    if (!allowModelFallback && recoverable && transientNetworkFailure && !timeoutLikeFailure && !transientNetworkRetryUsed) {
                        ...
                        await emitHostStatus(run, "Binary is retrying once after a transient gateway/network failure.", attachedRes, {
                    ...
                    const canAttemptFallback = (() => {
                        if (openRouterFreeFallbackAllowed)
                            return true;
                        if (explicitCodexCapacityFallbackAllowed)
                            return true;
                        ...
                        if (forceNoTimeoutRetryChain) {
                            if (!allowSingleTimeoutFallbackForForced)
                                return false;
                            if (!timeoutLikeFailure)
                                return false;
                            if (forcedTimeoutFallbackUsed)
                                return false;
                            forcedTimeoutFallbackUsed = true;
                            return true;
                        }
                        if (timeoutLikeFailure)
                            return run.request.detach !== true;
                        return true;
                    })();
                    ...
                    await emitHostStatus(run, openRouterFreeFallbackAllowed
                        ? `Binary is switching to the next OpenRouter free model because the current one is rate-limited.`
                        : explicitCodexCapacityFallbackAllowed
                            ? isPortalBridgeAuthError(error)
                                ? `Binary is switching from Codex to your backup model because the Codex bridge needs to be signed in again.`
                                : `Binary is switching from Codex to your backup model because Codex is temporarily busy.`
                            : `Binary is retrying the first OpenHands turn with fallback model candidate (${nextCandidate.displayName || nextCandidate.alias}).`, attachedRes, {
            ...
            try {
                ...
                envelope = (await runWithTransportRetry(run, attachedRes, () => streamHostedAssist({
                    baseUrl: preferences.baseUrl,
                    apiKey: auth.apiKey,
                    request: {
                        ...run.request,
                        ...
                        delegation: run.request.delegation && run.request.delegation.enabled === false
                            ? { ...DEFAULT_HOSTED_DELEGATION_CONFIG, enabled: false }
                            : run.request.delegation || DEFAULT_HOSTED_DELEGATION_CONFIG,
                    },
                    onEvent: async (event) => {
                ...
                await emitHostStatus(run, "Binary is falling back to the local OpenHands gateway because the hosted compatibility path is temporarily unavailable.", attachedRes, {
                    reason: error instanceof Error ? error.message : String(error),
        ...
        await persistHostRun(run);
        await agentJobManager.syncFromRun(run);
        if (envelope.userInputRequest) {
            await appendRunEvent(run, {
                event: "request_user_input",
                data: envelope.userInputRequest,
            }, attachedRes);
            await pauseRun(run, attachedRes, "Binary Host is waiting for user input before continuing the plan.");
            return;
        }
        ...
        if (shouldForceInitialWorkspaceBootstrap({
            task: run.request.task,
            taskSpeedClass,
            workspaceRoot: run.workspaceRoot,
            ...
            if (bootstrapCall) {
                ...
                await emitHostStatus(run, "Binary is adding a deterministic workspace bootstrap step because this non-chat run finished before executing any tool action.", attachedRes, {
                    bootstrapTool: bootstrapCall.toolCall.name,
                    bootstrapTarget: typeof bootstrapCall.toolCall.arguments.path === "string"
        ...
        if (!envelope.pendingToolCall &&
            ...
            if (validationCommand) {
                ...
                await emitHostStatus(run, "Binary is running baseline validation before accepting completion so the agent can repair from concrete failures.", attachedRes, {
                ...
                if (fallbackBootstrapCall) {
                    ...
                    await emitHostStatus(run, "Binary could not infer a validation command, so it is forcing a deterministic workspace bootstrap step first.", attachedRes, {
                        bootstrapTool: fallbackBootstrapCall.toolCall.name,
                        bootstrapTarget: typeof fallbackBootstrapCall.toolCall.arguments.path === "string"
        ...
        while (envelope.pendingToolCall && (assistTransport === "local_gateway" || envelope.runId)) {
            ...
            if (run.workspaceRoot &&
                ...
                await repoModelService.recordVerification(run.workspaceRoot, {
                    label: pendingToolCall.toolCall.name === "repo_record_verification"
                    ...
                    status: toolResult.ok ? "passed" : "failed",
                    command: typeof pendingToolCall.toolCall.arguments.command === "string"
                        ? pendingToolCall.toolCall.arguments.command
                        : undefined,
                    failureCategory: typeof toolResult.data?.failureCategory === "string"
                        ? toolResult.data.failureCategory
                        : toolResult.ok
                            ? undefined
                            : "validation_command_failure",
                    targetHint: typeof pendingToolCall.toolCall.arguments.path === "string"
            ...
            const shouldForceMutationDirective = !runLikelyDesktopTask(run) &&
            ...
            const repairDirective = shouldForceMutationDirective
                ? {
                    ...
                    reason: "Task requires concrete workspace changes or validation execution; do not end after observation-only steps.",
                }
                : null;
            ...
            try {
                envelope =
                    assistTransport === "local_gateway" && localGatewayModelCandidate
                        ...
                        : (await runWithTransportRetry(run, attachedRes, () => continueHostedRun({
                            baseUrl: preferences.baseUrl,
                            apiKey: auth.apiKey,
                ...
                const openRouterFreeFallbackAllowed = assistTransport === "local_gateway" &&
                    shouldAttemptOpenRouterFreeFallback({
                        current: localGatewayModelCandidate,
                        candidates: localGatewayModelCandidates,
                        error,
                        attemptedAliases: attemptedFallbackAliases,
                    });
                const explicitCodexCapacityFallbackAllowed = assistTransport === "local_gateway" &&
                    shouldAttemptExplicitCodexCapacityFallback({
                        current: localGatewayModelCandidate,
                        candidates: localGatewayModelCandidates,
                        error,
                        attemptedAliases: attemptedFallbackAliases,
                ...
                if ((openRouterFreeFallbackAllowed || explicitCodexCapacityFallbackAllowed) && localGatewayModelCandidate) {
                    const nextCandidate = selectNextModelCandidate(localGatewayModelCandidate, localGatewayModelCandidates, attemptedFallbackAliases);
                    if (nextCandidate) {
                        attemptedFallbackAliases.add(nextCandidate.alias);
                        localGatewayModelCandidate = nextCandidate;
                        localGatewayModelCandidates = [
                        ...
                        markEscalation(run, "fallback_model_candidate", `Switching to ${nextCandidate.alias} after OpenRouter free-tier capacity was exhausted.`);
                        run.lastExecutionState = {
                            ...(run.lastExecutionState || {}),
                        ...
                        await emitHostStatus(run, openRouterFreeFallbackAllowed
                            ...
                            : isPortalBridgeAuthError(error)
                                ? "Binary is switching from Codex to your backup model because the Codex bridge needs to be signed in again."
                                : "Binary is switching from Codex to your backup model because Codex is temporarily busy.", attachedRes, {
                            escalationStage: "fallback_model_candidate",
                            reason: error instanceof Error ? error.message : String(error),
                            modelAlias: nextCandidate.alias,
                            adapterMode: nextCandidateHints.adapterMode,
                        ...
                        }), Date.now() - continuePlannerStartedAt);
                    }
                    else {
                        throw error;
                    }
                }
                else if (!isTurnBudgetTimeoutError(error)) {
                    throw error;
                }
                else {
                    const timeoutReason = error instanceof Error
                        ? error.message
                    ...
                    const missionCandidate = turnRoutePolicy.missionFirstBrowser && isBrowserMicroToolName(pendingToolCall.toolCall.name)
                        ? buildBrowserMissionToolCall(run.request.task, run.toolResults.length + 1)
                        : null;
                    if (missionCandidate) {
                        markEscalation(run, "browser_mission", "Budget timeout triggered mission-first browser recovery.");
                        envelope = {
                            ...envelope,
                            ...
                            progressState: {
                                ...
                                nextDeterministicAction: "Run browser_search_and_open_best_result to collapse open/search/click into a single mission step.",
                            },
                        };
                        applyEnvelopeToRun(run, envelope);
                        updatePendingStats(run, envelope);
                    ...
                    if (assistTransport === "local_gateway" && allowModelFallback && localGatewayModelCandidates.length > 1) {
                        ...
                        if (nextCandidate) {
                            ...
                            const nextCandidateHints = gatewayHintsForCandidate(nextCandidate, false);
                            markEscalation(run, "fallback_model_candidate", `Switching to ${nextCandidate.alias} after timeout on ${pendingToolCall.toolCall.name}.`);
                            run.lastExecutionState = {
                                ...(run.lastExecutionState || {}),
                                latencyTier: nextCandidate.latencyTier,
                                intendedUse: nextCandidate.intendedUse,
                            ...
                            await emitHostStatus(run, `Binary is trying a fallback model candidate (${nextCandidate.displayName || nextCandidate.alias}) after a timeout.`, attachedRes, {
                                escalationStage: "fallback_model_candidate",
                                reason: timeoutReason,
                                modelAlias: nextCandidate.alias,
                                adapterMode: nextCandidateHints.adapterMode,
                            ...
                            const blockedEnvelope = buildEscalationBlockedEnvelope({
                                ...
                                nextDeterministicAction: "Use Take over and run the next concrete action manually, then resume for closure.",
                            });
            ...
            if (stallReason) {
                ...
                const missionEscalationCandidate = turnRoutePolicy.missionFirstBrowser && envelope.pendingToolCall?.toolCall?.name
                    ? isBrowserMicroToolName(envelope.pendingToolCall.toolCall.name)
                        ? buildBrowserMissionToolCall(run.request.task, run.toolResults.length + 1)
                        : null
                    : null;
                if (missionEscalationCandidate) {
                    markEscalation(run, "browser_mission", "Stall detected in browser micro-steps; switching to mission-first browser action.");
                    envelope = {
                        ...envelope,
                        ...
                        progressState: {
                            ...
                            status: "in_progress",
                            stallReason,
                            nextDeterministicAction: "Run browser_search_and_open_best_result to complete the browser intent in one action.",
                        },
                    };
                    applyEnvelopeToRun(run, envelope);
                    updatePendingStats(run, envelope);
                ...
                if (assistTransport === "local_gateway" && allowModelFallback && localGatewayModelCandidates.length > 1) {
                    ...
                    if (nextCandidate) {
                        ...
                        const nextCandidateHints = gatewayHintsForCandidate(nextCandidate, false);
                        markEscalation(run, "fallback_model_candidate", `Stall persisted, switching to fallback model ${nextCandidate.alias}.`);
                        run.lastExecutionState = {
                            ...(run.lastExecutionState || {}),
                            latencyTier: nextCandidate.latencyTier,
                            intendedUse: nextCandidate.intendedUse,
                        ...
                        await emitHostStatus(run, `Binary is switching to fallback model ${nextCandidate.displayName || nextCandidate.alias}.`, attachedRes, {
                            escalationStage: "fallback_model_candidate",
                            reason: stallReason,
                            modelAlias: nextCandidate.alias,
                            adapterMode: nextCandidateHints.adapterMode,
                ...
                const blockedEnvelope = buildEscalationBlockedEnvelope({
                    ...
                    reason: stallReason,
                    stage: "blocked_after_escalation",
                    nextDeterministicAction: "Use Take over and execute the next required step manually, then resume to let Binary close out with proof.",
                });
        ...
        if (run.workspaceRoot && !runLikelyDesktopTask(run) && taskRequestsValidation(run.request.task) && !hasSuccessfulCommandProof(run)) {
            ...
            if (validationCommand) {
                ...
                if (!validationProbeResult.ok) {
                    const baselineFailureReason = typeof validationProbeResult.summary === "string" && validationProbeResult.summary.trim()
                    ...
                    const shouldAttemptDeterministicRepair = assistTransport === "local_gateway" &&
                        Boolean(localGatewayModelCandidate) &&
                        taskLikelyRequiresWorkspaceAction(run.request.task);
                    if (shouldAttemptDeterministicRepair && localGatewayModelCandidate) {
                        await emitHostStatus(run, "Binary is forcing one deterministic repair turn after failed validation before requiring takeover.", attachedRes, {
                            escalationStage: "validation_repair_turn",
                            reason: baselineFailureReason,
                        });
                        try {
                            const forcedRepairStartedAt = Date.now();
                            const forcedRepairEnvelope = withEnvelopeLatency(await runLocalGatewayAssistWithHints({
                                run,
                                modelCandidate: localGatewayModelCandidate,
                                modelCandidates: localGatewayModelCandidates,
                                desktopContext,
                                ...
                                repairDirective: {
                                    stage: "post_inspection_mutation_required",
                                    reason: "Validation failed. Produce one concrete workspace mutation before attempting validation again.",
                                },
                                onEvent: async (event) => {
                                    await appendRunEvent(run, event, attachedRes);
                                },
                            ...
                            if (envelope.pendingToolCall) {
                                ...
                                let forcedRepairToolName = String(envelope.pendingToolCall.toolCall.name || forcedRepairResult.name || "");
                                ...
                                if (!forcedRepairResult.ok || !isWorkspaceMutationToolName(forcedRepairToolName)) {
                                    const shouldRetrySingleFileRewrite = assistTransport === "local_gateway" &&
                                        Boolean(localGatewayModelCandidate) &&
                                        taskLikelyRequiresWorkspaceAction(run.request.task) &&
                                    ...
                                    if (shouldRetrySingleFileRewrite && localGatewayModelCandidate) {
                                        await emitHostStatus(run, "Binary is retrying with a deterministic single-file rewrite repair strategy.", attachedRes, {
                                        ...
                                        try {
                                            ...
                                            const rewriteRepairEnvelope = withEnvelopeLatency(await runLocalGatewayAssistWithHints({
                                                run,
                                                modelCandidate: localGatewayModelCandidate,
                                                modelCandidates: localGatewayModelCandidates,
                                                desktopContext,
                                                ...
                                                repairDirective: {
                                                    ...
                                                    reason: "Previous mutation attempt failed or was non-mutation; emit one concrete single-file rewrite/edit for the broken target.",
                                                },
                                                onEvent: async (event) => {
                                                    await appendRunEvent(run, event, attachedRes);
                                                },
                                            ...
                                            if (envelope.pendingToolCall) {
                                                ...
                                                forcedRepairToolName = String(envelope.pendingToolCall.toolCall.name || rewriteRepairResult.name || forcedRepairToolName);
                                            }
                                            else {
                                                await emitHostStatus(run, "Binary requested a single-file rewrite retry but no repair tool call was produced.", attachedRes, {
                                                    escalationStage: "validation_repair_single_file_rewrite_no_call",
                                                });
                                            }
                                        }
                                        catch (error) {
                                            await emitHostStatus(run, `Binary could not complete the single-file rewrite retry. Cause: ${error instanceof Error ? error.message : String(error)}`, attachedRes, {
                                    ...
                                    if (!forcedRepairResult.ok || !isWorkspaceMutationToolName(forcedRepairToolName)) {
                                        ...
                                        await finalizeRun(run, "takeover_required", attachedRes, {
                                            message: "Binary Host paused because deterministic repair turns did not produce a successful workspace mutation.",
                                        });
                                ...
                                if (revalidationCommand) {
                                    ...
                                    run.takeoverReason = reason;
                                    await finalizeRun(run, "takeover_required", attachedRes, {
                                        message: "Binary Host paused because validation still failed after the forced deterministic repair turn.",
                                    });
                                    await emitTakeoverRequired(run, reason, attachedRes);
                                    return;
                                }
                                ...
                                await emitHostStatus(run, "Binary requested a deterministic repair turn but no mutation tool call was produced; retrying with single-file rewrite.", attachedRes, {
                                    escalationStage: "validation_repair_missing_call",
                                });
                                let rewriteSeedResult = validationProbeResult;
                                ...
                                const rewriteRepairEnvelope = withEnvelopeLatency(await runLocalGatewayAssistWithHints({
                                    ...
                                    repairDirective: {
                                        stage: "single_file_rewrite",
                                        reason: "No repair tool call was emitted; produce one concrete single-file rewrite/edit against the broken target.",
                                    },
                                    onEvent: async (event) => {
                                        await appendRunEvent(run, event, attachedRes);
                                    },
                                ...
                                if (envelope.pendingToolCall) {
                                    ...
                                    await emitHostStatus(run, "Binary single-file rewrite retry still produced no tool call.", attachedRes, {
                                        escalationStage: "validation_repair_single_file_rewrite_no_call",
                                    });
                                }
                            }
                            ...
                            await emitHostStatus(run, `Binary could not complete the forced deterministic repair turn. Cause: ${error instanceof Error ? error.message : String(error)}`, attachedRes, {
        ...
        const message = error instanceof Error ? error.message : String(error);
        const openRouterFreeExhaustionMessage = summarizeOpenRouterFreeExhaustion({
            current: localGatewayModelCandidate,
            candidates: localGatewayModelCandidates,
            attemptedAliases: attemptedFallbackAliases,
            error,
        });
        const publicMessage = openRouterFreeExhaustionMessage || message;
        run.error = publicMessage;
        run.updatedAt = nowIso();
        if (openRouterFreeExhaustionMessage) {
            await emitHostStatus(run, openRouterFreeExhaustionMessage, attachedRes, {
                failureReason: "openrouter_free_capacity_exhausted",
                rawError: message,
            });
        }
        if (run.runId || run.toolResults.length > 0) {
            run.status = "takeover_required";
            run.takeoverReason = publicMessage;
            await persistHostRun(run);
            await emitTakeoverRequired(run, publicMessage, attachedRes);
            await emitHostStatus(run, openRouterFreeExhaustionMessage
                ? openRouterFreeExhaustionMessage
                : `Binary Host preserved the run for takeover after an execution failure. Cause: ${message}`, attachedRes, { error: publicMessage, ...(openRouterFreeExhaustionMessage ? { rawError: message } : {}) });
            return;
        }
        await finalizeRun(run, "failed", attachedRes, {
            error: publicMessage,
            message: openRouterFreeExhaustionMessage || "Binary Host failed on the local OpenHands path.",
        });
    }
    finally {
        ...
        try {
            ...
            await emitHostStatus(run, `Binary could not finish desktop app cleanup. Cause: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`, attachedRes);
        }
...
async function executeBinaryInspectorTool(name, argumentsValue) {
    const moduleRef = (await import("../../../sdk/playground-ai-cli/dist/tool-executor.js"));
...
async function handleBinaryHttpTool(req, res, toolName) {
    const body = await readJsonBody(req);
    const result = await executeBinaryInspectorTool(toolName, body);
    if (!result.ok) {
...
async function handleAssist(req, res) {
    ...
    const request = {
        ...
        executionLane: body.executionLane === "local_interactive" ||
        ...
        delegation: normalizeDelegationConfig(body.delegation),
...
async function handleRunUserInput(req, res, runId) {
    const run = await loadRunRecord(runId);
    if (!run) {
    ...
    if (activeExecutions.has(run.id)) {
        writeJson(res, 409, { error: "Run active", message: "The run is still active." });
        return;
    }
    const body = await readJsonBody(req);
    const requestId = String(body.requestId || "").trim();
    const pendingRequest = run.pendingUserInputRequest;
    if (!pendingRequest || !requestId || pendingRequest.requestId !== requestId) {
        writeJson(res, 409, {
            error: "No pending user input",
            message: "This run is not waiting for the specified user-input request.",
        });
        return;
    }
    const answers = normalizeUserInputAnswers(body.answers);
    if (Object.keys(answers).length === 0) {
        writeJson(res, 400, { error: "Invalid request", message: "answers are required" });
        return;
    }
    const capturedAt = nowIso();
    ...
    const attachedRes = wantsStream ? res : null;
    await appendRunEvent(run, {
    ...
    const finalReceipt = run.finalEnvelope?.receipt && typeof run.finalEnvelope.receipt === "object"
        ? run.finalEnvelope.receipt
        : null;
    const receiptEngine = typeof finalReceipt?.engine === "string" ? String(finalReceipt.engine) : "";
    const canResumeHostedRun = Boolean(run.runId && run.sessionId) && receiptEngine !== "local_openhands_gateway";
    if (canResumeHostedRun) {
        const preferences = await loadPreferences();
        const auth = await getApiKeyRecord();
        if (!auth.apiKey) {
            if (wantsStream) {
                await emitHostStatus(run, "Binary Host could not resume because no API key is configured.", attachedRes);
                if (!res.writableEnded) {
                    res.write("data: [DONE]\n\n");
                    res.end();
                }
                ...
                writeJson(res, 503, {
                    error: "Hosted auth missing",
                    message: "Binary Host could not resume because no API key is configured.",
                });
            }
            return;
        }
        run.pendingUserInputRequest = null;
        run.userInputResponses = [
            ...(run.userInputResponses || []),
            {
                requestId,
                answers,
        ...
        run.takeoverReason = undefined;
        run.error = undefined;
        run.updatedAt = capturedAt;
        await persistHostRun(run);
        await agentJobManager.syncFromRun(run);
        const hostedEnvelope = await submitHostedUserInput({
            baseUrl: preferences.baseUrl,
            apiKey: auth.apiKey,
            runId: run.runId,
            requestId,
            sessionId: run.sessionId,
            answers: Object.fromEntries(Object.entries(answers).map(([key, value]) => [key, [...value.answers]])),
        }, {
            fetchTimeoutMs: 90_000,
        });
        applyEnvelopeToRun(run, hostedEnvelope);
        updatePendingStats(run, hostedEnvelope);
        await persistHostRun(run);
        await agentJobManager.syncFromRun(run);
        await appendRunEvent(run, {
            event: "meta",
            data: attachHostMetadata(hostedEnvelope, run),
        }, attachedRes);
        if (hostedEnvelope.userInputRequest) {
            await appendRunEvent(run, {
                event: "request_user_input",
                data: hostedEnvelope.userInputRequest,
            }, attachedRes);
            await pauseRun(run, attachedRes, "Binary Host is waiting for more user input before continuing the plan.");
        }
        else {
            if (hostedEnvelope.final) {
                await appendRunEvent(run, {
                    event: "final",
                    data: hostedEnvelope.final,
                }, attachedRes);
            }
            await finalizeRun(run, "completed", attachedRes, {
                message: "Binary Host resumed the plan after user input.",
            });
        }
        if (wantsStream && !res.writableEnded) {
            ...
            writeJson(res, 200, buildRunSummary(run));
        }
        return;
    }
    run.request = {
        ...run.request,
        task: appendPlanUserInputContext(run.request.task, pendingRequest, answers),
        historySessionId: run.sessionId || run.request.historySessionId,
    };
    run.pendingUserInputRequest = null;
    run.userInputResponses = [
        ...(run.userInputResponses || []),
...
async function streamExistingRun(runId, res, after = 0) {
    ...
    while (!res.destroyed && !res.writableEnded) {
        ...
        if (isTerminalStatus(run.status) || run.status === "takeover_required" || run.status === "paused") {
            break;
        }
...
async function streamAutomationEvents(automationId, res, after = 0) {
    writeSseHeaders(res);
    let lastSeq = after;
    while (!res.destroyed && !res.writableEnded) {
...
async function getMergedAgentJobEvents(jobId, after = 0) {
    ...
    return {
        job: response.job,
        events: combined,
        done: response.done || Boolean(run && (isTerminalStatus(run.status) || run.status === "takeover_required" || isWaitingForUserInput(run))),
    };
}
...
async function upsertConnectionRecord(body) {
    const preferences = await loadPreferences();
    const recordBody = body && typeof body === "object" ? body : {};
    const bodyId = typeof recordBody.id === "string" ? recordBody.id.trim() : "";
    const existing = bodyId
...
const server = createServer(async (req, res) => {
    ...
    try {
        ...
        if (method === "POST" && url.pathname === "/v1/providers/connect/oauth/poll") {
            ...
            let provider = poll?.status === "connected"
        ...
        if (method === "POST" && providerActionMatch) {
            const providerId = decodeURIComponent(providerActionMatch[1] || "");
        ...
        if (method === "GET" && url.pathname === "/v1/world-model/beliefs") {
            ...
            writeJson(res, 200, {
                beliefs: await worldModelService.getBeliefs({
                    ...
                    status: statusRaw === "active" || statusRaw === "stale" || statusRaw === "expired" || statusRaw === "contradicted"
                        ? statusRaw
                        : undefined,
                    limit,
                }),
        ...
        if (method === "GET" && url.pathname === "/v1/world-model/goals") {
            ...
            writeJson(res, 200, {
                goals: await worldModelService.getGoals({
                    ...
                    status: statusRaw === "open" || statusRaw === "in_progress" || statusRaw === "blocked" || statusRaw === "completed"
                        ? statusRaw
                        : undefined,
                    limit,
                }),
        ...
        if (method === "POST" && url.pathname === "/v1/world-model/goals/register") {
            ...
            writeJson(res, 200, await worldModelService.registerGoal({
                ...
                entityIds: Array.isArray(body.entityIds) ? body.entityIds.map((item) => String(item)) : [],
                progress: typeof body.progress === "number" ? body.progress : undefined,
                confidence: typeof body.confidence === "number" ? body.confidence : undefined,
                subgoals: Array.isArray(body.subgoals) ? body.subgoals.map((item) => String(item)) : [],
            }));
            return;
        }
        ...
        if (method === "POST" && url.pathname === "/v1/preferences") {
            ...
            const next = {
                ...
                orchestrationPolicy: body.orchestrationPolicy && typeof body.orchestrationPolicy === "object"
                    ? normalizeOrchestrationPolicy({
                        ...current.orchestrationPolicy,
                        ...body.orchestrationPolicy,
                    }, defaultOrchestrationPolicy())
                    : current.orchestrationPolicy,
                preferredExecutionLane: body.preferredExecutionLane === null
                    ? undefined
                    : body.preferredExecutionLane === "local_interactive" ||
                        body.preferredExecutionLane === "openhands_headless" ||
                        body.preferredExecutionLane === "openhands_remote"
                        ? body.preferredExecutionLane
                        : current.preferredExecutionLane,
                ...
                machineRootMode: body.machineRootMode === "home_root" || body.machineRootMode === "hybrid_root"
                    ? body.machineRootMode
                    : current.machineRootMode,
                machineTrustMode: body.machineTrustMode === "observe_first" ||
                    body.machineTrustMode === "home_mutate" ||
                    body.machineTrustMode === "full_machine_mutate"
                    ? body.machineTrustMode
                    : current.machineTrustMode,
                systemPathScope: body.systemPathScope === "excluded" || body.systemPathScope === "included" || body.systemPathScope === "prompt"
                    ? body.systemPathScope
                    : current.systemPathScope,
                focusWorkspaceRoot: typeof body.focusWorkspaceRoot === "string"
                    ? normalizeOptionalPath(body.focusWorkspaceRoot)
                    : current.focusWorkspaceRoot,
                focusRepoRoot: typeof body.focusRepoRoot === "string" ? normalizeOptionalPath(body.focusRepoRoot) : current.focusRepoRoot,
        ...
        if (method === "GET" && eventsMatch) {
            ...
            writeJson(res, 200, {
                ...
                done: isTerminalStatus(run.status) || run.status === "takeover_required" || isWaitingForUserInput(run),
            });
            return;
        }
        ...
        const userInputMatch = url.pathname.match(/^\/v1\/runs\/([^/]+)\/user-input$/);
        if (method === "POST" && userInputMatch) {
            await handleRunUserInput(req, res, decodeURIComponent(userInputMatch[1] || ""));
            return;
        }
        ...
        if (method === "POST" && url.pathname === "/v1/debug/agent-sessions") {
            ...
            if (typeof body.message === "string" && body.message.trim()) {
                const updated = await agentProbeManager.submitMessage(session.id, { message: body.message.trim() }, executeHostedAgentProbeTurn);
        ...
        if (method === "POST" && agentProbeMessageMatch) {
            ...
            const session = await agentProbeManager.submitMessage(sessionId, { message }, executeHostedAgentProbeTurn);
            if (!session) {
                writeJson(res, 404, { error: "Not found", message: "Unknown agent probe session." });
                return;
            }
        ...
        if (method === "POST" && agentProbeControlMatch) {
            ...
            const body = await readJsonBody(req);
            const action = body.action === "pause" || body.action === "resume" || body.action === "close"
                ? body.action
                : null;
            if (!action) {
