...
import { AutomationRuntime, automationToLegacyAgent, interpretAutomationSchedule, legacyAgentToAutomation, } from "./automation-runtime.js";
...
async function createRuntimeHarness(initialConfig, fetchImpl) {
...
describe("automation-runtime", () => {
    ...
    it("debounces file events and ignores default excluded paths", async () => {
        ...
        await runtime.handleFileChange(workspaceRoot, "src/app.ts", "change");
        await runtime.handleFileChange(workspaceRoot, "src/app.ts", "change");
        await runtime.handleFileChange(workspaceRoot, "node_modules/pkg/index.js", "change");
        const fileDebounce = runtime.fileDebounce;
    ...
    it("deduplicates repeated process ticks until the active window changes", async () => {
        ...
        runtime.input.getDesktopSnapshot = async () => snapshots[snapshotIndex++] || snapshots.at(-1);
        await runtime.runProcessTick();
        await runtime.runProcessTick();
        await runtime.runProcessTick();
        expect(queuedRuns).toHaveLength(1);
    ...
    it("delivers signed webhooks and records healthy delivery state", async () => {
        ...
        await runtime.flushDeliveries();
        expect(requests).toHaveLength(1);
        const sentBody = requests[0]?.body || "";
        const headers = new Headers(requests[0]?.headers);
        expect(headers.get("X-Binary-Event")).toBe("automation.run_completed");
        expect(headers.get("X-Binary-Signature")).toBe(createHmac("sha256", "secret-key").update(sentBody).digest("hex"));
    ...
    it("maps legacy background agents into automations and back", () => {
        ...
        expect(automation.trigger.scheduleText).toBe("every 30 minutes");
