...
describe("BrowserToolExecutor", () => {
    ...
    it("resolves a human-readable page reference before querying elements", async () => {
        const runtime = {
            ...
            queryElements: async (_policy, input) => ({
                page: {
                    id: input.pageId,
                    title: "YouTube",
        ...
        const executor = new BrowserToolExecutor(runtime, buildPolicy());
        const result = await executor.execute(buildPendingToolCall("browser_query_elements", {
    ...
    it("runs a cohesive browser search mission through one tool", async () => {
        const runtime = {
            currentSessionKind: () => "existing",
            searchAndOpenBestResult: async (_policy, input) => ({
                searchPage: {
                    id: "page_search",
        ...
        const executor = new BrowserToolExecutor(runtime, buildPolicy());
        const result = await executor.execute(buildPendingToolCall("browser_search_and_open_best_result", {
            url: "https://www.youtube.com/",
            query: "outdoor boys",
        }));
        expect(result.ok).toBe(true);
    ...
    it("falls back to the active page for browser keypresses when pageId is omitted", async () => {
        ...
        const executor = new BrowserToolExecutor(runtime, buildPolicy());
        const result = await executor.execute(buildPendingToolCall("browser_press_keys", {
            keys: ["Enter"],
        }));
        expect(result.ok).toBe(true);
        expect(result.data?.proof).toEqual(expect.objectContaining({
    ...
    it("runs a cohesive browser login mission", async () => {
        ...
        const executor = new BrowserToolExecutor(runtime, buildPolicy());
        const result = await executor.execute(buildPendingToolCall("browser_login_and_continue", {
            url: "https://example.com/login",
        ...
        expect(result.data?.missionLease).toEqual(expect.objectContaining({
            leaseId: "lease_login",
            missionKind: "browser_login_and_continue",
        }));
    });
    it("runs a cohesive browser form mission", async () => {
        ...
        const executor = new BrowserToolExecutor(runtime, buildPolicy());
        const result = await executor.execute(buildPendingToolCall("browser_complete_form", {
            pageId: "active",
            fields: [
    ...
    it("runs a browser extract-and-decide mission", async () => {
        ...
        const executor = new BrowserToolExecutor(runtime, buildPolicy());
        const result = await executor.execute(buildPendingToolCall("browser_extract_and_decide", {
            query: "outdoor boys",
    ...
    it("runs a browser workflow recovery mission", async () => {
        ...
        const executor = new BrowserToolExecutor(runtime, buildPolicy());
        const result = await executor.execute(buildPendingToolCall("browser_recover_workflow", {
    ...
    it("enforces screenshot policy for browser_capture_page when no reason is provided", async () => {
        ...
        const executor = new BrowserToolExecutor(runtime, buildPolicy());
        const result = await executor.execute(buildPendingToolCall("browser_capture_page", {
            pageId: "page_1",
        }));
        expect(result.ok).toBe(false);
        expect(result.blocked).toBe(true);
    ...
    it("enforces browser target guards before mutation tools", async () => {
        ...
        const runtime = {
            ...
            assertPageTarget: async (_policy, input) => {
        ...
        const executor = new BrowserToolExecutor(runtime, buildPolicy());
        const result = await executor.execute(buildPendingToolCall("browser_click", {
            pageId: "page_1",
    ...
    it("switches to managed-only browser preference while the user is actively focused", async () => {
        ...
        const runtime = {
            currentSessionKind: () => "existing",
            runWithSessionPreference: async (mode, action) => {
                observedMode = mode;
                return await action();
            },
        ...
        const executor = new BrowserToolExecutor(runtime, policy, controller);
        const result = await executor.execute(buildPendingToolCall("browser_search_and_open_best_result", {
            url: "https://www.youtube.com/",
            query: "outdoor boys",
        }));
