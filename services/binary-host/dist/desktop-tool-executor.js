...
export class DesktopToolExecutor {
    ...
    policy;
    executionController;
    constructor(machineAutonomyController, policy, executionController) {
    ...
    async execute(pendingToolCall) {
        ...
        if (String(toolCall.name || "").startsWith("desktop_")) {
            return fail(toolCall, `Binary Host has not implemented ${toolCall.name} yet. Prefer desktop_list_apps, desktop_open_app, desktop_open_url, desktop_list_windows, desktop_get_active_window, desktop_focus_window, or desktop_wait for now.`, true);
        }
