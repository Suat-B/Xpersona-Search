import { buildConnectionView, connectionHasRequiredSecret, } from "./connections.js";
const OPENROUTER_FREE_MODEL_LADDER = [
    {
...
const PROVIDER_CATALOG = [
    ...
    },
    {
        ...
        availabilityReason: "Binary can link an existing ChatGPT or Codex browser account session locally, but OpenAI direct API access remains API-key based.",
    ...
    },
    {
        ...
        availabilityReason: "Binary can link a local Qwen browser account session when Qwen credentials are present on the machine, while DashScope direct API access remains API-key based.",
        supportsLocalImport: true,
...
export function isProviderConnection(record) {
    return Boolean(record.providerId && getProviderCatalogEntry(record.providerId));
}
export function buildProviderAlias(providerId) {
    return `user:${String(providerId || "").trim().toLowerCase()}`;
}
function buildProviderModelAlias(providerId, model) {
...
export function getProviderConnectionName(entry) {
    return `${entry.displayName} model provider`;
}
function inferProviderLatencyTier(providerId) {
    switch (providerId) {
        case "groq":
            return "fast";
        case "openrouter":
...
function inferProviderReasoningDefault(providerId) {
    switch (providerId) {
        case "openrouter":
        case "groq":
            return "low";
...
function inferProviderIntendedUse(providerId) {
    switch (providerId) {
        case "anthropic":
            return "repair";
        case "chatgpt_portal":
...
function buildProviderCandidateTemplates(input) {
    if (input.provider.id === "openrouter") {
        ...
        const configuredIsFreeModel = configuredModel.endsWith(":free") ||
            configuredModel === "openrouter/free" ||
            OPENROUTER_FREE_MODEL_LADDER.some((entry) => entry.model === configuredModel);
        const explicitOrder = new Map(OPENROUTER_FREE_MODEL_LADDER.map((entry, index) => [entry.model, index]));
        ...
        const liveFreeModels = Array.isArray(input.record.availableModels)
            ...
            : [];
        const mergedModels = (configuredModel && !configuredIsFreeModel
            ? [
                configuredModel,
                ...availableModels.filter((model) => !model.endsWith(":free") && model !== "openrouter/free"),
            ]
            : [
        ...
        const rankedModels = mergedModels.sort((left, right) => {
            if (configuredModel && left === configuredModel)
                return -1;
            if (configuredModel && right === configuredModel)
                return 1;
        ...
        return rankedModels.map((model, index) => {
            ...
            const inferredLatencyTier = known?.latencyTier ||
                (/(flash|mini|nano|1\.2b|2b|3b|4b|e2b)/i.test(model)
                    ? "fast"
                    : /(120b|405b|70b|80b)/i.test(model)
                        ? "thorough"
                        : "balanced");
            const inferredIntendedUse = known?.intendedUse ||
    ...
    const configuredModel = String(input.record.defaultModel || input.provider.defaultModel).trim() || input.provider.defaultModel;
    const availableModels = Array.isArray(input.record.availableModels)
        ? input.record.availableModels.map((item) => String(item || "").trim()).filter(Boolean)
        : [];
    const orderedModels = [configuredModel, ...availableModels].filter((model, index, list) => Boolean(model) && list.indexOf(model) === index);
...
export function buildProviderProfile(input) {
    ...
    return {
        ...
        supportsBrowserAuth: input.catalog.connectionMode === "direct_oauth_pkce" ||
        ...
        authReady: Boolean(input.record && connectionHasRequiredSecret(input.record, input.secret)) &&
...
export function listProviderProfiles(input) {
    ...
    return PROVIDER_CATALOG
        ...
        .map((catalog) => buildProviderProfile({
        catalog,
...
export function buildUserConnectedModelCandidates(input) {
    const candidates = input.records
        ...
        const credential = String(secret?.accessToken || "").trim() ||
            String(secret?.sessionToken || "").trim() ||
            String(secret?.apiKey || "").trim() ||
            String(secret?.bearerToken || "").trim();
        if (!credential)
            return [];
        const extraHeaders = { ...(secret?.secretHeaders || {}) };
        ...
        const modelFamilies = Array.isArray(record.modelFamilies) && record.modelFamilies.length
