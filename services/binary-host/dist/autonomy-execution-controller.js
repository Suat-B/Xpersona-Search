...
function isDesktopObserveTool(name) {
    return (name === "desktop_list_apps" ||
...
function isBrowserObserveTool(name) {
    return (name === "browser_list_pages" ||
...
function isBrowserInteractiveTool(name) {
    return (name === "browser_open_page" ||
...
function wantsVisibleForeground(toolCall) {
    const args = toolCall.arguments || {};
    return (asBoolean(args.allowForeground) ||
...
export class AutonomyExecutionController {
    ...
    decide(pendingToolCall) {
        ...
        const focusLeaseActive = Boolean(focusLease) &&
        ...
        if (focusLeaseActive &&
            ...
            return {
                ...
                visibleFallbackReason: decision.visibleFallbackReason ||
                    "Binary will not steal focus while an active desktop focus lease is held.",
            };
        }
        if (decision.executionVisibility === "visible_required" &&
            ...
            return {
                ...
                visibleFallbackReason: decision.visibleFallbackReason || "Automatic visible fallback is disabled by host policy.",
            };
        }
        return decision;
    }
    buildReceipt(decision, input) {
        const focusStolen = typeof input?.focusStolen === "boolean"
            ? input.focusStolen
