...
describe("WorldToolExecutor", () => {
    ...
    it("records memory and scores routes against affordances", async () => {
        ...
        const committed = await executor.execute(buildPendingToolCall("world_commit_memory", {
            label: "Preferred repo validation",
            summary: "Prefer npm test inside the repo root.",
            scope: "workspace",
            tags: ["node", "validation"],
        }));
        const scored = await executor.execute(buildPendingToolCall("world_score_route", {
            routes: [
        ...
        const routes = Array.isArray((scored.data || {}).routes) ? (scored.data || {}).routes : [];
    ...
    it("exposes beliefs, goals, episodes, predictions, and explanations through world tools", async () => {
        ...
        const predictions = await executor.execute(buildPendingToolCall("world_predict_outcomes", {
            candidates: [
        ...
        expect((goals.data?.goals)[0]?.title).toContain("Inspect inbox workflow");
    ...
    it("exposes learned route stats and explicit route outcomes through world tools", async () => {
        ...
        const recorded = await executor.execute(buildPendingToolCall("world_record_route_outcome", {
        ...
        expect((stats.data?.stats)[0]?.routeKind).toBe("terminal");
        expect(((stats.data?.stats)[0]?.successRate || 0)).toBeGreaterThan(0.5);
    });
});
