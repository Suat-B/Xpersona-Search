    ...
    async execute(pendingToolCall) {
        ...
        if (toolCall.name === "world_get_beliefs") {
            const beliefs = await this.worldModel.getBeliefs({
                ...
                status: args.status === "active" || args.status === "stale" || args.status === "expired" || args.status === "contradicted"
                    ? args.status
                    : undefined,
        ...
        if (toolCall.name === "world_get_goals") {
            const goals = await this.worldModel.getGoals({
                status: args.status === "open" || args.status === "in_progress" || args.status === "blocked" || args.status === "completed"
                    ? args.status
                    : undefined,
        ...
        if (toolCall.name === "world_register_goal") {
            ...
            if (!title)
                return fail(toolCall, "world_register_goal requires title.");
            ...
            return ok(toolCall, `Registered goal ${goal.title}.`, goal);
        }
        ...
        if (toolCall.name === "world_explain_route") {
            ...
            return ok(toolCall, "Explained the current world-model route preference.", explanation);
        }
        if (toolCall.name === "world_record_route_outcome") {
            const outcome = args.outcome === "success" ||
                ...
                : null;
            if (!outcome)
                return fail(toolCall, "world_record_route_outcome requires a valid outcome.");
            const record = await this.worldModel.recordRouteOutcome({
                ...
                verificationStatus: args.verificationStatus === "passed" || args.verificationStatus === "failed" ? args.verificationStatus : "unknown",
                fallbackToRouteKind: typeof args.fallbackToRouteKind === "string" ? args.fallbackToRouteKind : undefined,
                summary: typeof args.summary === "string" ? args.summary : undefined,
            });
            return ok(toolCall, `Recorded ${record.outcome} route outcome for ${record.routeKind}.`, record);
        }
