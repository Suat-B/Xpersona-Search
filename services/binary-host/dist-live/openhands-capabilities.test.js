...
describe("openhands-capabilities", () => {
    ...
    it("enables sampled tracing for headless and debug work", () => {
        expect(shouldEnableSampledTracing({
    ...
    it("returns unavailable remote health when no remote runtime is configured", async () => {
        ...
        try {
            ...
            if (previousRemoteGateway)
                process.env.OPENHANDS_REMOTE_GATEWAY_URL = previousRemoteGateway;
            if (previousAgentServer)
                process.env.OPENHANDS_AGENT_SERVER_URL = previousAgentServer;
        }
    });
