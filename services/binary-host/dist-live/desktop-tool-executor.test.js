...
describe("DesktopToolExecutor", () => {
    ...
    it("reuses existing app windows in background mode without active-window lookup", async () => {
        ...
        const result = await executor.execute(buildPendingToolCall("desktop_open_app", {
            app: "Notepad",
    ...
    it("blocks dangerous native invoke actions without explicit user authorization", async () => {
        ...
        const result = await executor.execute(buildPendingToolCall("desktop_invoke_control", {
            app: "Discord",
            selector: { name: "Send", controlType: "Button" },
        }));
        expect(result.ok).toBe(false);
    ...
    it("allows dangerous native invoke actions when the task explicitly authorizes them", async () => {
        ...
        const executor = new DesktopToolExecutor(controller, buildPolicy(), undefined, nativeRuntime, 'Launch Discord and message Sam "Hi thanks for dinner last night"', {
            deps: {
                listWindows: async () => [
                    {
                        id: "300",
    ...
    it("marks calculator verification as failed when display does not match expected result", async () => {
        ...
        const executor = new DesktopToolExecutor(controller, buildPolicy(), undefined, nativeRuntime, "Launch Calculator and tell me what 9*9 is");
        const result = await executor.execute(buildPendingToolCall("desktop_read_control", {
            app: "Calculator",
    ...
    it("supports chained calculator task phrasing for verification", async () => {
        ...
        const executor = new DesktopToolExecutor(controller, buildPolicy(), undefined, nativeRuntime, "Open Calculator and compute 12*12, then plus 10, and tell me the final result.");
        const result = await executor.execute(buildPendingToolCall("desktop_read_control", {
            app: "Calculator",
            targetAppIntent: "Calculator",
            query: "result display",
