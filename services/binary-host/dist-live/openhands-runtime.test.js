...
describe("openhands runtime supervisor helpers", () => {
    ...
    it("marks strict terminal lane as repair-needed when only fallback terminal is available", () => {
        const status = runtime.mapGatewayHealthToOpenHandsRuntimeStatus({
            gatewayUrl: "http://127.0.0.1:8010",
            desiredProfile: "full",
        ...
        expect(status.readiness).toBe("repair_needed");
        expect(status.message).toContain("Terminal runtime needs repair");
    ...
    it("detects native terminal availability from supported tools and degraded reasons", () => {
        ...
        expect(runtime.resolveNativeTerminalAvailability({
            ...
            terminalHealthReason: "terminal_tool_fallback_windows",
        });
    });
