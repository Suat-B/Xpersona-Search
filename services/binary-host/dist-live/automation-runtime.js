...
function encodeSignature(secret, payload) {
    return createHmac("sha256", secret).update(payload).digest("hex");
}
function matchesEventFilter(eventName, filters) {
    if (!filters?.length)
        return true;
    return filters.some((item) => item === "*" || item === eventName);
}
function fingerprintActiveWindow(activeWindow) {
    return compactWhitespace([activeWindow?.app, activeWindow?.title].filter(Boolean).join("|")).toLowerCase();
}
function triggerWorkspaceRoot(trigger) {
...
function parseNaturalLanguageHour(raw) {
...
function readState(filePath) {
    return fs
        ...
        processState: toRecord(parsed.processState) ? parsed.processState : {},
    }))
        .catch(() => ({ ...DEFAULT_STATE, version: 1 }));
}
async function writeState(filePath, state) {
    await fs.mkdir(path.dirname(filePath), { recursive: true });
    await fs.writeFile(filePath, JSON.stringify(state, null, 2), "utf8");
}
export function interpretAutomationSchedule(scheduleText, now = new Date(), lastTriggeredAt) {
    return computeNextScheduleOccurrence(scheduleText, now, lastTriggeredAt)?.toISOString() || null;
}
function computeNextScheduleOccurrence(scheduleText, now, lastTriggeredAt) {
...
function shouldTriggerSchedule(scheduleText, now, lastTriggeredAt) {
    if (!lastTriggeredAt)
        return true;
...
function pathMatchesFilters(relativePath, includes, excludes) {
    const normalized = relativePath.replace(/\\/g, "/").toLowerCase();
    if (DEFAULT_FILE_IGNORE.some((pattern) => pattern.test(normalized)))
        return false;
    if (excludes?.some((entry) => normalized.includes(entry.toLowerCase())))
        return false;
    if (!includes?.length)
        return true;
...
export function automationToLegacyAgent(automation) {
    let trigger = "manual";
    let scheduleMinutes;
...
export class AutomationRuntime {
    ...
    async saveAutomation(raw) {
        ...
        const automation = {
            ...
            trigger,
            policy: raw.policy === "observe_only" || raw.policy === "approval_before_mutation"
                ? raw.policy
    ...
    async saveWebhookSubscription(raw) {
        await this.initialize();
        const config = await this.input.readConfig();
        ...
        const next = {
            ...
            automationId: typeof raw.automationId === "string" && raw.automationId.trim() ? raw.automationId.trim() : existing?.automationId,
    ...
    async runAutomation(id, triggerSummary = "Manual automation run requested.") {
        await this.initialize();
    ...
    async ingestNotification(input) {
        ...
        for (const automation of config.automations) {
            if (automation.status !== "active" || automation.trigger.kind !== "notification")
                continue;
            if (input.automationId && automation.id !== input.automationId)
                continue;
            const matchesTopic = !automation.trigger.topic || automation.trigger.topic === topic;
            const matchesQuery = !automation.trigger.query ||
    ...
    async applyRunResult(input, eventName, severity, summary) {
        const config = await this.input.readConfig();
    ...
    normalizeTrigger(trigger, fallbackWorkspaceRoot) {
    ...
    decorateAutomationForReturn(automation, now) {
        if (automation.trigger.kind !== "schedule_nl") {
            return automation;
        }
        const nextRunAt = computeNextScheduleOccurrence(automation.trigger.scheduleText, now, automation.lastTriggerAt)?.toISOString();
        return {
            ...automation,
            nextRunAt,
        };
    }
    async emitAutomationEvent(input) {
        await this.initialize();
        ...
        this.state.events[input.automation.id] = [...(this.state.events[input.automation.id] || []), stored].slice(-MAX_AUTOMATION_EVENTS);
    ...
    async enqueueDeliveries(payload) {
        const config = await this.input.readConfig();
        const eventName = String(payload.event || "");
        for (const subscription of config.webhookSubscriptions) {
            if (subscription.status !== "active")
                continue;
            if (subscription.automationId && subscription.automationId !== String(payload.automationId || ""))
                continue;
            if (!matchesEventFilter(eventName, subscription.events))
                continue;
    ...
    async triggerAutomation(automation, input) {
        const config = await this.input.readConfig();
    ...
    async runSchedulerTick() {
        ...
        const now = new Date();
        for (const automation of config.automations) {
            if (automation.status !== "active" || automation.trigger.kind !== "schedule_nl")
                continue;
            if (!shouldTriggerSchedule(automation.trigger.scheduleText, now, automation.lastTriggerAt))
                continue;
            await this.triggerAutomation(automation, {
    ...
    async runProcessTick() {
        ...
        if (!fingerprint || fingerprint === this.state.processState.lastFingerprint)
            return;
        this.state.processState.lastFingerprint = fingerprint;
        await this.persistState();
        for (const automation of config.automations) {
            if (automation.status !== "active" || automation.trigger.kind !== "process_event")
                continue;
            if (!fingerprint.includes(automation.trigger.query.toLowerCase()))
                continue;
            await this.triggerAutomation(automation, {
    ...
    async syncFileWatchers() {
        const config = await this.input.readConfig();
        const roots = new Set();
        for (const automation of config.automations) {
            if (automation.status !== "active" || automation.trigger.kind !== "file_event")
                continue;
            if (!config.trustedWorkspaceRoots.includes(normalizePath(automation.trigger.workspaceRoot)))
                continue;
        ...
        for (const root of roots) {
            if (this.watchers.has(root))
                continue;
            try {
                const watcher = watch(root, { recursive: process.platform === "win32" }, (eventType, filename) => {
    ...
    async handleFileChange(root, relativePath, eventType) {
        const config = await this.input.readConfig();
        for (const automation of config.automations) {
            if (automation.status !== "active" || automation.trigger.kind !== "file_event")
                continue;
            if (normalizePath(automation.trigger.workspaceRoot) !== normalizePath(root))
                continue;
            if (!pathMatchesFilters(relativePath, automation.trigger.includes, automation.trigger.excludes))
                continue;
    ...
    async flushDeliveries() {
        ...
        for (const delivery of this.state.deliveries) {
            if (delivery.status === "delivered")
                continue;
            if (new Date(delivery.nextAttemptAt).getTime() > now)
                continue;
