import { inferBrowserMissionUrlFromQuery, stripBrowserSiteHintFromQuery, } from "./browser-runtime.js";
function nowIso() {
...
function missionLeasePayload(value) {
    if (!value || typeof value !== "object")
        return undefined;
    const record = value;
    if (typeof record.leaseId !== "string" || typeof record.pageId !== "string")
        return undefined;
    return {
...
export class BrowserToolExecutor {
    ...
    policy;
    executionController;
    constructor(runtime, policy, executionController) {
    ...
    shouldForceForeground(toolName, args) {
        ...
        if (executionMode === "foreground_lease")
            return true;
        if (executionMode === "background_safe")
            return false;
    ...
    async resolvePageId(pageRef, allowActiveFallback = false) {
        ...
        const fuzzyMatches = pages.filter((page) => pageReferenceCandidates(page).some((candidate) => candidate.includes(normalized) || normalized.includes(candidate)));
        ...
        const available = pages
            ...
            .join("; ");
        throw new Error(`Browser page ${raw} was not found. Use browser_list_pages or browser_get_active_page and pass an existing page id. Available pages: ${available}`);
    }
    async execute(pendingToolCall) {
        ...
        const runtimeWithSessionPreference = this.runtime;
        const runMission = async (operation) => typeof runtimeWithSessionPreference.runWithSessionPreference === "function"
            ? await runtimeWithSessionPreference.runWithSessionPreference(decision?.browserSessionPreference ?? (decision?.managedSessionPreferred ? "managed_only" : null), operation)
            : await operation();
        ...
        try {
            ...
            const lease = missionLeasePayload(error?.browserMissionLease);
            if (lease) {
                failure.data = {
                    lane: "browser_native",
                    missionLease: lease,
